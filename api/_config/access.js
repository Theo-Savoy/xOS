...
export function trackingModeFor(sfUserId, overrides = {}) {
  ...
  const fromOverride = Object.entries(overrides || {}).find(
    ([id]) => sfIdKey(id) === key,
  )?.[1];
  if (
    fromOverride === 'sdr' ||
    fromOverride === 'dg' ||
    fromOverride === 'commercial'
  )
    return fromOverride;
  const fromDefault = Object.entries(WEEKLY_TRACKING_BY_SF_USER).find(
    ([id]) => sfIdKey(id) === key,
  )?.[1];
...
export function isWeeklyOwnerExcluded(
  sfUser,
  nameFallback = '',
  emailFallback = '',
) {
  ...
  return WEEKLY_EXCLUDED_NAME_PATTERNS.some(
...
export function resolveCallsTeamLabel(
  sfUserId,
  fallbackLabel = '',
  email = '',
) {
  const key = sfIdKey(sfUserId);
  const override = Object.entries(CALLS_TEAM_LABEL_BY_SF_ID).find(
    ([id]) => sfIdKey(id) === key,
  )?.[1];
  if (override) return override;
  const fallback = String(fallbackLabel || '').trim();
  if (fallback && !fallback.includes('@') && !looksLikeSalesforceId(fallback))
    return fallback;
  const local =
    String(email || (fallback.includes('@') ? fallback : '') || '').split(
      '@',
    )[0] || '';
...
export function hasCallsTeamLabelOverride(sfUserId) {
  const key = sfIdKey(sfUserId);
  return Object.keys(CALLS_TEAM_LABEL_BY_SF_ID).some(
    (id) => sfIdKey(id) === key,
  );
}
...
export function isCallsAccountOwnerCandidate(sfUserId) {
  ...
  const key = sfIdKey(sfUserId);
  return !CALLS_ACCOUNT_OWNER_EXCLUDED_SF_PREFIXES.some((prefix) =>
