...
async function saveTargets(client, value) {
  const { error } = await client
    .from('settings')
    .upsert({ key: TARGETS_KEY, value }, { onConflict: 'key' });
...
function targetForOwner(targets, sfUserId, quarterLabel) {
  const entry = Object.entries(targets || {}).find(
    ([id]) => sfIdKey(id) === sfIdKey(sfUserId),
  )?.[1];
...
async function managerContext(request) {
  ...
  if (!canManageSettings(profile.role))
    return { response: respond(403, { error: 'forbidden' }) };
...
export async function GET(request) {
  ...
  const monthTemplate =
    qKey && FISCAL_QUARTER_MONTHS[qKey]
      ? FISCAL_QUARTER_MONTHS[qKey].map((month) => ({
          month,
          weight:
            seasonality?.month_in_quarter?.[qKey]?.[month] ??
            1 / FISCAL_QUARTER_MONTHS[qKey].length,
        }))
      : [];
  ...
  const rows = sellers.map((person) => {
    ...
    return {
      ...
      name:
        person.full_name || person.email?.split('@')[0] || person.sf_user_id,
      email: person.email,
      role: person.role,
      quarterly_target: quarterly,
      monthly_indicative: quarterly
        ? quarterlyToMonthlyIndicative(quarterly, quarter.label, seasonality)
        : [],
    };
  });
  ...
  return respond(200, {
    quarter: {
      label: quarter.label,
      from: quarter.from,
      to: quarter.toExclusive,
    },
    seasonality: seasonality
      ? { as_of: seasonality.as_of, sample_years: seasonality.sample_years }
      : null,
...
export async function POST(request) {
  ...
  const quarterLabel =
    typeof body.quarter === 'string' ? body.quarter.trim() : null;
  ...
  for (const [sfUserId, raw] of Object.entries(values)) {
    ...
    const existing =
      Object.entries(targets).find(([id]) => sfIdKey(id) === key)?.[1] || {};
    ...
    const storedKey =
      Object.keys(targets).find((id) => sfIdKey(id) === key) || sfUserId;
