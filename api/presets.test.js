...
function makeReq(
  method,
  body,
  url = 'http://localhost/api/calls?resource=presets',
) {
...
describe('parsePresetId', () => {
  it('accepts safe positive integers', () => {
    ...
    expect(parsePresetId(Number.MAX_SAFE_INTEGER)).toBe(
...
describe('validatePresetInput', () => {
  it('rejects invalid body and filters families', () => {
    ...
    expect(validatePresetInput({ name: '', filters: {} }).error).toBe(
      'invalid_name',
    );
    expect(
      validatePresetInput({ name: 'X', filters: { relance: [] } }).error,
    ).toBe('invalid_filters');
    expect(
      validatePresetInput({ name: 'X', filters: {}, shared: 'yes' }).error,
    ).toBe('invalid_shared');
  });
});
  ...
  it('returns 400 for invalid id', async () => {
    const res = await DELETE(
      makeReq(
        'DELETE',
        undefined,
        'http://localhost/api/calls?resource=presets&id=1abc',
      ),
    );
    expect(res.status).toBe(400);
  });
  ...
  it('returns 404 when preset missing', async () => {
    ...
    const res = await DELETE(
      makeReq(
        'DELETE',
        undefined,
        'http://localhost/api/calls?resource=presets&id=3',
      ),
    );
