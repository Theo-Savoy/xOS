...
describe('Salesforce token cache and paginated queries', () => {
  ...
  it('returns a cached token on the second successful call', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ access_token: 'cached-token' }));
    vi.stubGlobal('fetch', fetchMock);

    await expect(fetchSFToken()).resolves.toEqual({
  ...
  it('refreshes the token when forceRefresh is requested or the TTL expires', async () => {
    ...
    await fetchSFToken();
    await expect(fetchSFToken({ forceRefresh: true })).resolves.toEqual({
  ...
  it('follows query pages and truncates records at the fetch cap', async () => {
    const firstPage = Array.from({ length: 1_999 }, (_, index) => ({
      Id: String(index),
    }));
    const fetchMock = vi
      .fn()
    ...
    expect(result).toEqual({
      records: [...firstPage, { Id: 'last' }],
      truncated: true,
    });
    expect(fetchMock.mock.calls[1][0]).toBe(
      'https://instance.example.test/next',
    );
  });
  ...
  it('retries a query once with a refreshed token after a 401', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({ records: [{ Id: '003' }], done: true }),
      );
    vi.stubGlobal('fetch', fetchMock);

    await expect(
      searchContacts('stale-token', 'SELECT Id FROM Contact'),
    ).resolves.toEqual({ records: [{ Id: '003' }], truncated: false });
  ...
  it('retries Salesforce writes with a refreshed token', async () => {
    ...
    await expect(
      logCall('stale-token', { contactId: '003', resultat: 'Appel décroché' }),
    ).resolves.toEqual({ record: { id: '00T' } });
    await expect(
      updateContactDoNotCall('stale-token', '003', true),
