...
async function verifyIdentityAndStore({
  client,
  profile,
  refreshToken,
  accessToken,
  ...
  if (
    !emailMatches ||
    !profile.sf_user_id ||
    identity.user_id !== profile.sf_user_id
  ) {
...
export async function storeSalesforceRefreshToken({
  client,
  user,
  refreshToken,
}) {
...
export async function startSalesforceOAuth({ client, user, origin }) {
  ...
  const { error } = await client
    ...
    .update({
      ...
      sf_oauth_state_expires_at: new Date(
...
export async function completeSalesforceOAuth({ client, url }) {
  ...
  if (!tokens.access_token || !tokens.refresh_token || !tokens.instance_url)
    return { error: 'sf_refresh_token_missing' };
