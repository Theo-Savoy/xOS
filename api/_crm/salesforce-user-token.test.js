...
import { decryptRefreshToken, encryptRefreshToken } from './tokenEncryption.js';
...
describe('Salesforce per-user credentials', () => {
  ...
  it('encrypts refresh tokens with authenticated encryption', async () => {
    ...
    await expect(decryptRefreshToken(encrypted)).resolves.toBe(
      'user-refresh-token',
    );
  ...
  it("uses and caches the linked user's refresh token", async () => {
    ...
    const { client, maybeSingle } = profileClient({
      sf_refresh_token_encrypted: ciphertext,
    });
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ access_token: 'ada-access' }));
    vi.stubGlobal('fetch', fetchMock);

    await expect(
      fetchSFToken({ client, userId: 'user-ada' }),
  ...
  it('fails closed when the user is not linked (no org fallback)', async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ access_token: 'integration-access' }));
    vi.stubGlobal('fetch', fetchMock);

    await expect(
      fetchSFToken({ client, userId: 'user-unlinked' }),
    ).resolves.toEqual({ error: 'sf_auth_error' });
  ...
  it('allows explicit org fallback when requested', async () => {
    ...
    await expect(
      fetchSFToken({ client, userId: 'user-unlinked', allowOrgFallback: true }),
  ...
  it('uses the same per-user retry path for composite updates', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse([{ id: '006', success: true, errors: [] }]),
      );
    vi.stubGlobal('fetch', fetchMock);

    const token = await fetchSFToken({ client, userId: 'user-ada' });
    await expect(
      updateSObjects(token.accessToken, 'Opportunity', [
        { id: '006', StageName: 'Gagnée' },
      ]),
