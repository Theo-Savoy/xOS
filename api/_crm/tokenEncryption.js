...
const ADDITIONAL_DATA = new TextEncoder().encode(
  'xos:salesforce-refresh-token:v1',
);
...
export async function encryptRefreshToken(refreshToken) {
  if (typeof refreshToken !== 'string' || !refreshToken)
    throw new Error('sf_refresh_token_invalid');
...
export async function decryptRefreshToken(encrypted) {
  const [version, ivPart, ciphertextPart, extra] = String(
    encrypted || '',
  ).split('.');
  if (version !== VERSION || !ivPart || !ciphertextPart || extra)
    throw new Error('sf_refresh_token_ciphertext_invalid');
