...
describe('Salesforce OAuth account linking', () => {
  ...
  it('creates an authenticated authorization request without persisting the raw state', async () => {
    ...
    expect(authorization.origin).toBe(
      'https://db0000000d7rdeay.my.salesforce.com',
    );
    ...
    expect(updates[0].sf_oauth_state_hash).not.toBe(
  ...
  it('exchanges the code, verifies the Salesforce identity and stores an encrypted refresh token', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(
          response({ user_id: '005ADA', email: 'ADA@xos-learning.fr' }),
        ),
    );
    ...
    await expect(
      completeSalesforceOAuth({
        client,
        url: new URL(
          'https://xos.hellotheo.fr/api/auth?flow=salesforce-callback&code=code-1&state=state-1',
        ),
      }),
    ...
    await expect(
      decryptRefreshToken(stored.sf_refresh_token_encrypted),
  ...
  it('rejects a Salesforce identity that does not match the mapped profile', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(
          response({ user_id: '005OTHER', email: 'ada@xos-learning.fr' }),
        ),
    );
    ...
    expect(updates.some((value) => value.sf_refresh_token_encrypted)).toBe(
      false,
    );
  });
  ...
  it('validates and stores a provider refresh token received after Salesforce login', async () => {
    ...
    await expect(
      decryptRefreshToken(stored.sf_refresh_token_encrypted),
    ).resolves.toBe('provider-refresh');
  });
});
