...
export async function searchContacts(token, soql) {
  ...
  if (!response.ok) return { error: "sf_query_error", message: (await response.text()).slice(0, 500) };
  ...
  while (page.done === false && page.nextRecordsUrl && records.length < SOQL_FETCH_CAP) {
    ...
    const { response: nextResponse } = nextResult;
    if (!nextResponse.ok) return { error: "sf_query_error", message: (await nextResponse.text()).slice(0, 500) };
...
export async function fetchContactContext(token, { contactId, accountId }, mapping = defaultMapping) {
  ...
  const [tasksResult, oppResult] = await Promise.all([
    searchContacts(token, taskSoql),
    oppSoql ? searchContacts(token, oppSoql) : Promise.resolve(null),
  ]);
