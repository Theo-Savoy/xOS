...
export async function handleSessionWrite({
  ...
  if (action === 'create_session') {
    ...
    if (!Array.isArray(contacts) || contacts.length === 0) {
      return new Response(JSON.stringify({ error: 'invalid_contacts' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (scheduledForInput !== undefined) {
      if (!isValidScheduledFor(scheduledForInput)) {
        return new Response(
          JSON.stringify({ error: 'invalid_scheduled_for' }),
          { status: 400, headers },
        );
      }
    ...
    const sessionType =
      sessionTypeInput === undefined ? 'prospection' : sessionTypeInput;
    if (!isValidSessionType(sessionType)) {
      return new Response(JSON.stringify({ error: 'invalid_session_type' }), {
        status: 400,
        headers,
      });
    }
    for (let i = 0; i < contacts.length; i++) {
      ...
      if (
        ...
        contact.contact_name.trim().length === 0
      ) {
        return new Response(JSON.stringify({ error: 'invalid_contact_name' }), {
          status: 400,
          headers,
        });
      }
      if (
        contact.sf_account_id !== undefined &&
        contact.sf_account_id !== null &&
        (typeof contact.sf_account_id !== 'string' ||
          !SF_ID.test(contact.sf_account_id))
      ) {
        return new Response(
          JSON.stringify({ error: 'invalid_sf_account_id' }),
          { status: 400, headers },
        );
      }
    }
    ...
    if (memberUserIdsInput !== undefined) {
      if (
        !Array.isArray(memberUserIdsInput) ||
        memberUserIdsInput.some((id) => typeof id !== 'string' || !id)
      ) {
        return new Response(
          JSON.stringify({ error: 'invalid_member_user_ids' }),
          { status: 400, headers },
        );
      }
      uniqueMemberIds = [
        ...new Set(
          memberUserIdsInput.filter(
            (id) => id !== user.id && !String(id).startsWith('map:'),
          ),
        ),
      ];
      if (uniqueMemberIds.length > 0) {
        ...
        if (profilesError) {
          return new Response(
            JSON.stringify({ error: 'members_lookup_failed' }),
            { status: 500, headers },
          );
        }
    ...
    if (uniqueMemberIds.length > 0) {
      ...
      if (insertError) {
        return new Response(
          JSON.stringify({ error: 'members_update_failed' }),
          { status: 500, headers },
        );
      }
  ...
  if (action === 'create_audience_sessions') {
    ...
    if (!Array.isArray(groups) || groups.length === 0) {
      return new Response(JSON.stringify({ error: 'invalid_groups' }), {
        status: 400,
        headers,
      });
    }
    for (const group of groups) {
      ...
      for (const contact of group.contacts) {
        ...
        if (
          ...
          return new Response(
            JSON.stringify({ error: 'invalid_contact_name' }),
            { status: 400, headers },
          );
        }
        if (
    ...
    const namePrefix =
      typeof namePrefixInput === 'string' && namePrefixInput.trim()
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      const name = namePrefix
        ? `${namePrefix} #${i + 1}`
        : nextContinuationName(`Audience #${i}`);
      const created = await insertSessionWithContacts(
        client,
        user.id,
        name,
    ...
    return new Response(JSON.stringify({ sessions: createdSessions }), {
      status: 200,
      headers,
    });
  }
  ...
  if (action === 'update_session') {
    const {
      ...
      scheduled_for: scheduledForInput,
      session_type: sessionTypeInput,
      rdv_goal: rdvGoalInput,
      engaged_at: engagedAtInput,
    } = body;
    ...
    if (scheduledForInput !== undefined) {
      if (
        scheduledForInput !== null &&
        !isValidScheduledFor(scheduledForInput)
      ) {
        return new Response(
          JSON.stringify({ error: 'invalid_scheduled_for' }),
          { status: 400, headers },
    ...
    if (rdvGoalInput !== undefined) {
      if (
        !Number.isInteger(rdvGoalInput) ||
        rdvGoalInput < 1 ||
        rdvGoalInput > 8
      ) {
        return new Response(JSON.stringify({ error: 'invalid_rdv_goal' }), {
          status: 400,
          headers,
        });
      }
      if (
        sessionCheck.session.rdv_goal != null &&
        rdvGoalInput < sessionCheck.session.rdv_goal
      ) {
        return new Response(
          JSON.stringify({ error: 'rdv_goal_cannot_decrease' }),
          { status: 409, headers },
        );
      }
    ...
    if (engagedAtInput !== undefined) {
      if (
        typeof engagedAtInput !== 'string' ||
        Number.isNaN(Date.parse(engagedAtInput))
      ) {
        return new Response(JSON.stringify({ error: 'invalid_engaged_at' }), {
          status: 400,
          headers,
        });
      }
    ...
    if (
      rdvGoalInput !== undefined &&
      sessionCheck.session.engaged_at &&
      !engagedAtInput
    ) {
      patch.engaged_at = sessionCheck.session.engaged_at;
    }
    if (Object.keys(patch).length === 0) {
      return new Response(JSON.stringify({ error: 'empty_update' }), {
        status: 400,
        headers,
      });
    }
    ...
    const { data: updated, error: updateError } = await client
      ...
      .eq('id', session_id)
      .select(
        'id, name, status, created_at, scheduled_for, session_type, rdv_goal, engaged_at',
      )
      .single();

    if (updateError || !updated) {
      return new Response(JSON.stringify({ error: 'session_update_failed' }), {
        status: 500,
        headers,
      });
    }

    return new Response(JSON.stringify({ ok: true, session: updated }), {
      status: 200,
      headers,
    });
  }
  ...
  if (action === 'delete_session') {
    ...
    const { error: deleteError } = await client
      ...
      .eq('id', session_id);
    if (deleteError) {
      return new Response(JSON.stringify({ error: 'session_delete_failed' }), {
        status: 500,
        headers,
      });
    }
  ...
  if (action === 'create_follow_up_session') {
    ...
    const { data: sessionContacts, error: contactsLookupError } = await client
      ...
      .select(
        'sf_contact_id, sf_account_id, contact_name, account_name, phone, email, title, linkedin_url, outcome, status, attempt_count',
      )
    ...
    if (contactsLookupError) {
      return new Response(
        JSON.stringify({ error: 'session_contacts_lookup_failed' }),
        { status: 500, headers },
      );
    }
    ...
    if (followUpContacts.length === 0) {
      return new Response(JSON.stringify({ error: 'no_follow_up_contacts' }), {
        status: 400,
        headers,
      });
    }
  ...
  if (action === 'defer_contacts') {
    ...
    if (
      ...
      return new Response(JSON.stringify({ error: 'invalid_contact_ids' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (
      ...
      (typeof targetSessionId !== 'number' ||
        !Number.isInteger(targetSessionId) ||
        targetSessionId < 1)
    ) {
      return new Response(
        JSON.stringify({ error: 'invalid_target_session_id' }),
        { status: 400, headers },
      );
    }
    if (
    ...
    if (
      sessionTypeInput !== undefined &&
      !isValidSessionType(sessionTypeInput)
    ) {
      return new Response(JSON.stringify({ error: 'invalid_session_type' }), {
        status: 400,
        headers,
    ...
    const { data: sourceContacts, error: sourceError } = await client
      .from('call_session_contacts')
      .select(
        'id, sf_contact_id, sf_account_id, contact_name, account_name, phone, email, title, linkedin_url, status, attempt_count',
      )
      .eq('session_id', session_id)
      .in('id', contact_ids);
    ...
    if (!sourceContacts || sourceContacts.length !== contact_ids.length) {
      return new Response(JSON.stringify({ error: 'contact_not_in_session' }), {
        status: 404,
        headers,
      });
    }
    if (sourceContacts.some((contact) => contact.status !== 'pending')) {
      return new Response(JSON.stringify({ error: 'contact_not_pending' }), {
        status: 400,
        headers,
      });
    }
    ...
    const payloadContacts = sourceContacts.map((contact) => ({
      ...
      attempt_count: Number.isInteger(contact.attempt_count)
    ...
    if (typeof targetSessionId === 'number') {
      const targetCheck = await assertSessionOwner(
        client,
        targetSessionId,
        user.id,
      );
      if (targetCheck.error) {
        return new Response(JSON.stringify({ error: targetCheck.error }), {
      ...
      if (targetCheck.session.status !== 'active') {
        return new Response(
          JSON.stringify({ error: 'target_session_not_active' }),
          { status: 400, headers },
        );
      }
      ...
      if (existingError) {
        return new Response(
          JSON.stringify({ error: 'contacts_lookup_failed' }),
          { status: 500, headers },
        );
      }

      const existingIds = new Set(
        (existingRows || []).map((row) => row.sf_contact_id),
      );
      const toInsert = payloadContacts.filter(
        (contact) => !existingIds.has(contact.sf_contact_id),
      );
      const startPosition =
        existingRows?.[0]?.position != null ? existingRows[0].position + 1 : 0;
      ...
      if (toInsert.length > 0) {
        ...
        if (insertError) {
          return new Response(
            JSON.stringify({ error: 'session_contacts_insert_failed' }),
            { status: 500, headers },
          );
        }
      }
      ...
      const created = await insertSessionWithContacts(
        ...
        scheduledForInput,
        {
          ...
          sessionType:
            sessionTypeInput ||
            (sourceContacts.every(
              (contact) =>
                contact.status === 'pending' && !(contact.attempt_count > 0),
            )
              ? 'prospection'
              : 'relance'),
        },
  ...
  if (action === 'set_session_members') {
    ...
    if (
      ...
      memberUserIds.some((id) => typeof id !== 'string' || !id)
    ) {
      return new Response(
        JSON.stringify({ error: 'invalid_member_user_ids' }),
        { status: 400, headers },
    ...
    if (existingMembersError) {
      return new Response(
        JSON.stringify({ error: 'session_members_lookup_failed' }),
        { status: 500, headers },
      );
    }
    ...
    const { error: deleteError } = await client
      ...
      .eq('session_id', session_id);
    if (deleteError) {
      return new Response(JSON.stringify({ error: 'members_update_failed' }), {
        status: 500,
        headers,
      });
    }
    ...
    if (newlyAddedIds.length > 0) {
      const actorLabel =
        user.user_metadata?.full_name || user.email || 'Un collègue';
    ...
    return new Response(JSON.stringify({ ok: true, members: payload }), {
      status: 200,
      headers,
    });
  }
