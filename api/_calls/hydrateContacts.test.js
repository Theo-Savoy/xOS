...
import {
  ...
  hydrateSessionContactsFromCrm,
} from './hydrateContacts.js';
...
describe('hydrateSessionContactsFromCrm', () => {
  ...
  it('fills missing email and title from CRM and persists updates', async () => {
    ...
    const enriched = await hydrateSessionContactsFromCrm(
      client,
      contacts,
      'sf-token',
      mapping,
    ...
    expect(enriched[0].title).toBe(
      'Responsable du Département Innovations Tech',
    );
  ...
  it('does not retry a CRM lookup for an unchanged incomplete contact', async () => {
    ...
    const contacts = [
      { id: 1, sf_contact_id: '003AAA', email: null, title: null },
    ];
