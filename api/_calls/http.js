...
export function filterContactsForFollowUp(
  contacts,
  followUpOutcomes = getFollowUpOutcomes(),
) {
  return (Array.isArray(contacts) ? contacts : []).filter((contact) => {
    ...
    if (contact?.status === 'skipped' || contact?.status === 'pending')
      return true;
...
export function computeHubKpis(rows) {
  ...
  const decroche = called.filter((row) =>
    PIPE_DECROCHE.includes(row?.outcome),
  ).length;
  const argumente = called.filter((row) =>
    PIPE_ARGUMENTE.includes(row?.outcome),
  ).length;
...
export async function claimSessionContact(client, contact, userId) {
  ...
  if (!updated) {
    ...
    if (
      fresh.claimed_by &&
      fresh.claimed_by !== userId &&
      isClaimActive(fresh.claimed_at)
    ) {
      return {
        error: 'contact_claimed',
        status: 409,
...
export async function assertSessionContact(client, sessionId, contactId) {
  const { data: contact, error } = await client
    ...
    .maybeSingle();
  if (error && !isNotFoundError(error))
    return { error: 'contact_lookup_failed', status: 500 };
  if (!contact || contact.session_id !== sessionId)
    return { error: 'not_found', status: 404 };
  return { contact };
}

export async function insertSessionWithContacts(
  client,
  userId,
  name,
  contacts,
  ...
  const sessionType = isValidSessionType(options.sessionType)
    ? options.sessionType
    : 'prospection';
  ...
  if (sessionError || !session)
    return { error: 'session_creation_failed', status: 500 };
  ...
  const contactRows = contacts.map((contact, index) => ({
    ...
    status: 'pending',
    attempt_count:
      Number.isInteger(contact.attempt_count) && contact.attempt_count >= 0
