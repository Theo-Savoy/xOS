...
async function loadMembersBySessionIds(
  ...
  for (const [sessionId, ids] of bySession) {
    result.set(
      sessionId,
      ids.map(
        (id) => labels.get(id) || { user_id: id, label: id, sf_user_id: null },
      ),
    );
  }
  return result;
...
async function loadContactCountsBySessionIds(client, sessionIds) {
  ...
  const { data: rpcRows, error: rpcError } = await client.rpc(
    'call_session_contact_counts',
    {
      p_session_ids: sessionIds,
    },
  );
  const legacyRpcShape = (rpcRows || []).some(
    (row) => row && 'done' in row && !('called' in row),
  );
...
async function buildSessionSummaries(client, userId, accessibleIds) {
  ...
  const { data: sessions, error: sessionsError } = await client
    .from('call_sessions')
    .select(
      'id, owner, name, status, created_at, scheduled_for, session_type, rdv_goal, engaged_at',
    )
    .in('id', accessibleIds)
...
export async function handleSessionsRead({ url, user, client, headers }) {
  ...
  if (resource === 'presets') {
    ...
    return new Response(JSON.stringify({ presets: result.presets }), {
      status: 200,
      headers,
    });
  }
  ...
  if (resource === 'team') {
    ...
    if (profilesResult.error) {
      return new Response(JSON.stringify({ error: 'team_lookup_failed' }), {
        status: 500,
        headers,
      });
    }
    ...
    if (!mapResult.error) {
      for (const row of mapResult.data || []) {
        if (row.sf_user_id && row.email)
          emailBySfId.set(row.sf_user_id, row.email);
      }
    }
    ...
    if (!mapResult.error) {
      for (const row of mapResult.data || []) {
        ...
        const label = resolveCallsTeamLabel(
          row.sf_user_id,
          row.sf_user_id,
          row.email,
        );
    ...
    const team = [...bySfId.values()].sort((a, b) =>
  ...
  if (resource === 'hub') {
    ...
    if (accessible.error) {
      return new Response(JSON.stringify({ error: accessible.error }), {
        status: 500,
        headers,
      });
    }
    ...
    if (summaries.error) {
      return new Response(JSON.stringify({ error: summaries.error }), {
        status: 500,
        headers,
      });
    }
    if (statsResult.error) {
      return new Response(JSON.stringify({ error: statsResult.error }), {
        status: 500,
        headers,
      });
    }
    if (recallsResult.error) {
      return new Response(JSON.stringify({ error: recallsResult.error }), {
        status: 500,
        headers,
      });
    }
  ...
  if (resource === 'recalls') {
    ...
    if (recallsError) {
      return new Response(JSON.stringify({ error: 'recalls_lookup_failed' }), {
        status: 500,
        headers,
      });
    }
  ...
  if (statsParam === '1') {
    ...
    return new Response(JSON.stringify({ stats: statsResult.stats }), {
      status: 200,
      headers,
    });
  }
  ...
  if (sessionIdParam) {
    ...
    const [
      ...
      client
        .from('call_session_contacts')
        .select(
          'id, position, sf_contact_id, sf_account_id, contact_name, account_name, phone, email, title, linkedin_url, status, outcome, comments, sf_task_id, sf_event_id, called_at, recall_at, attempt_count, marked_npa, logged_by, claimed_by, claimed_at',
        )
    ...
    if (contacts.some((contact) => !contact.email || !contact.title)) {
      ...
      if (!tokenResult.error) {
        contacts = await hydrateSessionContactsFromCrm(
          client,
          contacts,
    ...
    const claimerIds = [
      ...new Set(contacts.map((c) => c.claimed_by).filter(Boolean)),
    ];
    ...
    if (contextContactId) {
      const row = (contacts || []).find(
        (c) => String(c.id) === String(contextContactId),
      );
      if (!row) {
        return new Response(
          JSON.stringify({ error: 'contact_not_in_session' }),
          { status: 404, headers },
        );
      }
      const tokenResult = await fetchSFToken({ client, userId: user.id });
  ...
  if (accessibleList.error) {
    return new Response(JSON.stringify({ error: accessibleList.error }), {
      status: 500,
      headers,
    });
  }
  const summaries = await buildSessionSummaries(
    client,
    user.id,
    accessibleList.ids,
  );
  ...
  return new Response(JSON.stringify({ sessions: summaries.sessions }), {
    status: 200,
    headers,
  });
}
