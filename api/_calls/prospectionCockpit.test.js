...
describe('prospection_cockpit', () => {
  ...
  it('returns 403 for commercial role', async () => {
    ...
    const res = await GET(
      makeReq('http://localhost/api/calls?resource=prospection_cockpit'),
    );
  ...
  it('aggregates team funnel and RDV attribution for managers', async () => {
    ...
    const res = await GET(
      makeReq(
        'http://localhost/api/calls?resource=prospection_cockpit&period=week',
      ),
    );
    ...
    expect(body.by_rdv_owner[0]).toMatchObject({
      label: 'Christophe',
      rdv: 1,
      from_sdr: 1,
    });
  ...
  it('anchors period range and keeps heatmap when anchor is set', async () => {
    ...
    const res = await GET(
      makeReq(
        'http://localhost/api/calls?resource=prospection_cockpit&period=day&anchor=2026-06-01',
      ),
    );
    expect(res.status).toBe(200);
    const body = await res.json();
