...
export async function findActiveSessionConflicts(client, contactIds, todayIso) {
  ...
  if (sessionContacts.length >= SESSION_CONTACTS_LIMIT) {
    console.warn(
      `findActiveSessionConflicts: session_contacts capped at ${SESSION_CONTACTS_LIMIT} rows`,
    );
  }
  ...
  for (const row of sessionContacts) {
    if (!conflicts.has(row.sf_contact_id)) {
      ...
      conflicts.set(
        row.sf_contact_id,
        ownerLabels.get(session.owner) || session.owner,
      );
    }
  }
  return [...conflicts].map(([sf_contact_id, in_session_of]) => ({
    sf_contact_id,
    in_session_of,
  }));
}
