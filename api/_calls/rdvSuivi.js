...
async function handleListRdvs({ body, user, client, headers }) {
  ...
  const teamSfUserIds =
    Array.isArray(body.team_sf_user_ids) && body.team_sf_user_ids.length > 0
      ? body.team_sf_user_ids.filter((id) => typeof id === 'string' && id)
      : null;

  const ownerIds =
    teamSfUserIds ?? (profileResult.sfUserId ? [profileResult.sfUserId] : []);
  if (ownerIds.length === 0) {
    return new Response(JSON.stringify({ rdvs: [], pending_count: 0 }), {
      status: 200,
      headers,
    });
  }
  ...
  const now = new Date();
  const rangeStart =
    body.range_start || new Date(now.getTime() - 7 * 86400_000).toISOString();
  const rangeEnd =
    body.range_end || new Date(now.getTime() + 30 * 86400_000).toISOString();
  ...
  return new Response(JSON.stringify({ rdvs, pending_count: pendingCount }), {
    status: 200,
    headers,
  });
}
...
async function handleReportRdv({ body, user, client, headers }) {
  ...
  if (typeof sf_event_id !== 'string' || !sf_event_id) {
    return new Response(JSON.stringify({ error: 'invalid_sf_event_id' }), {
      status: 400,
      headers,
    });
  }
  ...
  const { error: upsertError } = await client.from('rdv_followups').upsert(
    {
      sf_event_id,
      status,
      notes: notes || null,
  ...
  if (new_start && typeof new_start === 'string') {
    ...
    if (!Number.isNaN(start.getTime())) {
      const duration =
        Number.isFinite(Number(duration_min)) && Number(duration_min) > 0
          ? Number(duration_min)
          : 60;
      sfFields.StartDateTime = start.toISOString();
      sfFields.EndDateTime = new Date(
  ...
  if (Object.keys(sfFields).length > 1) {
    const sfResult = await updateSObjects(tokenResult.accessToken, 'Event', [
      sfFields,
    ]);
    if (sfResult.error) {
      return new Response(
...
export async function handleRdvSuivi({ action, body, user, client, headers }) {
  if (action === 'list_rdvs')
    return handleListRdvs({ body, user, client, headers });
  if (action === 'report_rdv')
    return handleReportRdv({ body, user, client, headers });
  return null;
}
