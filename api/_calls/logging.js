...
export async function handleLogging({ action, body, user, client, headers }) {
  if (action === 'log_call') {
    ...
    if (
      typeof contact_id !== 'number' ||
      !Number.isInteger(contact_id) ||
      contact_id < 1
    ) {
      return new Response(JSON.stringify({ error: 'invalid_contact_id' }), {
        status: 400,
        headers,
      });
    }
    if (!VALID_RESULTS.includes(resultat)) {
      return new Response(JSON.stringify({ error: 'invalid_resultat' }), {
        status: 400,
        headers,
      });
    }
    if (comments !== undefined && typeof comments !== 'string') {
      return new Response(JSON.stringify({ error: 'invalid_comments' }), {
        status: 400,
        headers,
      });
    }
    if (
      ...
      (!Number.isInteger(duration_sec) || duration_sec < 0)
    ) {
      return new Response(JSON.stringify({ error: 'invalid_duration_sec' }), {
        status: 400,
        headers,
      });
    }
    if (recall_at !== undefined && recall_at !== null) {
      if (
        typeof recall_at !== 'string' ||
        !/^\d{4}-\d{2}-\d{2}$/.test(recall_at)
      ) {
        return new Response(JSON.stringify({ error: 'invalid_recall_at' }), {
          status: 400,
          headers,
        });
      }
    ...
    if (do_not_call !== undefined && typeof do_not_call !== 'boolean') {
      return new Response(JSON.stringify({ error: 'invalid_do_not_call' }), {
        status: 400,
        headers,
      });
    }
    ...
    const contactCheck = await assertSessionContact(
      client,
      session_id,
      contact_id,
    );
    if (contactCheck.error) {
      return new Response(JSON.stringify({ error: contactCheck.error }), {
    ...
    if (contact.status && contact.status !== 'pending' && !isRecallRelog) {
      return new Response(
        JSON.stringify({ error: 'contact_already_processed' }),
        { status: 409, headers },
      );
    }
    ...
    if (do_not_call === true) {
      try {
        const npaResult = await updateContactDoNotCall(
          tokenResult.accessToken,
    ...
    const { error: updateError } = await client
      ...
      .update({
        ...
        attempt_count:
          (Number.isInteger(contact.attempt_count)
  ...
  if (action === 'log_event') {
    ...
    if (!isValidEventStart(start)) {
      return new Response(JSON.stringify({ error: 'invalid_start' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (
      ...
      !ALLOWED_DURATIONS.includes(duration_min)
    ) {
      return new Response(JSON.stringify({ error: 'invalid_duration_min' }), {
        status: 400,
        headers,
      });
    }
    if (
      ...
      (!Array.isArray(invitees) ||
        invitees.some((id) => typeof id !== 'string' || !SF_ID.test(id)))
    ) {
      return new Response(JSON.stringify({ error: 'invalid_invitees' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (
      ...
      return new Response(
        JSON.stringify({ error: 'invalid_owner_sf_user_id' }),
        { status: 400, headers },
      );
    }
    ...
    if (owner_sf_user_id) {
      ...
      if (ownerProfile?.sf_user_id) {
        ...
        if (!mapped?.sf_user_id) {
          return new Response(JSON.stringify({ error: 'owner_not_in_team' }), {
            status: 400,
            headers,
          });
        }
    ...
    if (
      ...
      if (ownerProfile?.id) {
        ...
        await insertUserNotification(client, {
          ...
          payload: {
            sf_event_id: eventId,
            sf_event_url: buildLightningUrl(
              mapping.objects.event.name,
              eventId,
            ),
            subject,
    ...
    return new Response(JSON.stringify({ ok: true, sf_event_id: eventId }), {
      status: 200,
      headers,
    });
  }
  ...
  if (action === 'skip_contact') {
    ...
    if (
      contactCheck.contact.status &&
      contactCheck.contact.status !== 'pending'
    ) {
      return new Response(
        JSON.stringify({ error: 'contact_already_processed' }),
        { status: 409, headers },
    ...
    const claim = await claimSessionContact(
      client,
  ...
  if (action === 'remove_contact') {
    ...
    if (status !== 'pending' && status !== 'skipped') {
      return new Response(JSON.stringify({ error: 'contact_not_removable' }), {
        status: 409,
        headers,
      });
    }
    ...
    if (deleteError) {
      return new Response(JSON.stringify({ error: 'contact_delete_failed' }), {
        status: 500,
        headers,
      });
    }
  ...
  if (action === 'update_recall') {
    ...
    if (
      recallAtInput !== null &&
      (typeof recallAtInput !== 'string' ||
        !/^\d{4}-\d{2}-\d{2}$/.test(recallAtInput))
    ) {
      return new Response(JSON.stringify({ error: 'invalid_recall_at' }), {
        status: 400,
        headers,
    ...
    if (contact.status !== 'called') {
      return new Response(JSON.stringify({ error: 'contact_not_called' }), {
        status: 409,
        headers,
      });
    }
    if (recallAtInput === null && !contact.recall_at) {
      return new Response(JSON.stringify({ error: 'recall_not_set' }), {
        status: 409,
        headers,
      });
    }
    ...
    return new Response(
      JSON.stringify({ ok: true, recall_at: recallAtInput }),
      { status: 200, headers },
    );
  }
  ...
  if (action === 'celebrate_goal') {
    ...
    if (
      typeof rdv_count !== 'number' ||
      !Number.isInteger(rdv_count) ||
      rdv_count < 1
    ) {
      return new Response(JSON.stringify({ error: 'invalid_rdv_count' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (session.owner && session.owner !== user.id)
      recipientIds.add(session.owner);
