...
export function parseAccountsSearchBody(body) {
  ...
  if (body.filters !== undefined && !isObject(body.filters))
    return { error: 'invalid_filters' };
  const filters = body.filters || {};
  if (q.length < 2 && !hasAnyRefineFilter(filters))
...
function accountRefineConditions(filters, account) {
  ...
  if (sectors.length)
    conditions.push(`${account.fields.industry} IN (${escapedList(sectors)})`);
  const employeeBands = stringList(filters.effectifs);
  if (employeeBands.length)
    conditions.push(
      `${account.fields.employeeCount} IN (${escapedList(employeeBands)})`,
    );
  const customerTypes = stringList(filters.type_client);
  if (customerTypes.length)
    conditions.push(
      `${account.fields.customerType} IN (${escapedList(customerTypes)})`,
    );
  const tiers = stringList(filters.tiers);
  if (tiers.length)
    conditions.push(`${account.fields.tier} IN (${escapedList(tiers)})`);
  const owners = stringList(filters.proprietaires);
  if (owners.length)
    conditions.push(`${account.fields.ownerId} IN (${escapedList(owners)})`);
  if (
    typeof filters.compte_principal === 'string' &&
    filters.compte_principal
  ) {
    conditions.push(
...
export async function searchAccounts(client, userId, body) {
  ...
  if (!alreadyRefined && hasAnyRefineFilter(parsed.filters)) {
    ...
    const allowed = new Set(
      refineResult.records.map((record) => record[af.id]),
    );
    accountRecords = accountRecords.filter((record) =>
      allowed.has(record[af.id]),
    );
  }

  if (hasOpportunityQueryFilters({ entreprise: parsed.filters })) {
    const oppSets = await fetchOpportunityAccountIdSets(token, mapping, {
      entreprise: parsed.filters,
    });
    if (oppSets.error) return { error: oppSets.error, status: 502 };
    const allowedIds = filterByOpenLost(
      accountRecords.map((record) => record[af.id]),
      oppSets,
      parsed.filters,
    );
  ...
  const allContactIds = accounts.flatMap((acc) =>
    acc.contacts.map((c) => c.sf_contact_id),
  );
  const conflicts = await findActiveSessionConflicts(
    client,
    allContactIds,
    parisToday(),
  );
  ...
  return {
    ...
    truncated:
      soslCapped || contactsResult.records.length >= MAX_CONTACTS_PER_QUERY,
  };
}
