...
export async function POST(request) {
  ...
  if (
    typeof body?.salesforce_refresh_token === 'string' &&
    body.salesforce_refresh_token
  ) {
    const client = getServiceClient();
    if (client) {
      ...
      if (result.error)
        console.error(`Salesforce automatic link failed: ${result.error}`);
    } else {
      console.error(
        'storeSalesforceRefreshToken failed: supabase client is null',
      );
    }
  }
