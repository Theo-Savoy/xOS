...
export async function GET(request) {
  ...
  if (searchResult.error) {
    return new Response(JSON.stringify({ error: searchResult.error }), {
      status: 502,
      headers: noStore,
    });
  }
  ...
  return new Response(
    JSON.stringify({
      error: null,
      results: normalizeSFResults(searchResult.records),
    }),
    {
      status: 200,
      headers: noStore,
...
export async function POST(request) {
  ...
  if (action === 'log_call') {
    ...
    if (!recordId || typeof recordId !== 'string' || !SF_ID.test(recordId)) {
      return new Response(JSON.stringify({ error: 'invalid_record_id' }), {
        status: 400,
        headers,
      });
    }
    if (!['Account', 'Contact', 'Opportunity'].includes(recordType)) {
      return new Response(JSON.stringify({ error: 'invalid_record_type' }), {
        status: 400,
        headers,
      });
    }
    if (
      ...
      return new Response(JSON.stringify({ error: 'missing_comments' }), {
        status: 400,
        headers,
      });
    }
    ...
    const userFullName =
      user.user_metadata?.full_name || user.email || 'Utilisateur Inconnu';
    ...
    const sfResult = await createRecord(
    ...
    return new Response(
      JSON.stringify({ error: null, success: true, taskId }),
      { status: 200, headers },
    );
  }
  ...
  if (action === 'create_contact') {
    ...
    if (!lastName || typeof lastName !== 'string' || lastName.length === 0) {
      return new Response(JSON.stringify({ error: 'missing_last_name' }), {
        status: 400,
        headers,
      });
    }
    ...
    if (
      accountId &&
      (typeof accountId !== 'string' || !SF_ID.test(accountId))
    ) {
    ...
    if (sfResult.error) {
      return new Response(
        JSON.stringify({
          error: 'sf_contact_creation_failed',
          message: sfResult.message,
        }),
        { status: 502, headers },
      );
    ...
    return new Response(
      JSON.stringify({ error: null, success: true, contactId }),
      { status: 200, headers },
    );
  }
