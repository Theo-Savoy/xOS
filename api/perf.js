  ...
  return new Response(JSON.stringify(body), {
    ...
    headers: {
      ...
      ...(Number.isFinite(timingMs)
        ? { 'Server-Timing': `perf;dur=${Math.round(timingMs)}` }
        : {}),
      ...extraHeaders,
    },
  });
...
function weekWindow(weeks) {
  ...
  const starts = Array.from({ length: weeks }, (_, index) =>
    addDays(currentMonday, (index - weeks + 1) * 7),
  );
  return {
    starts,
    from: starts[0],
...
function weekWindowAnchored(anchorMonday, weeks = 2) {
  ...
  const starts = Array.from({ length: weeks }, (_, index) =>
    addDays(monday, (index - weeks + 1) * 7),
  );
  return {
    starts,
    from: starts[0],
    to: addDays(monday, 6),
    toExclusive: addDays(monday, 7),
    period: 'week',
  };
}
...
export function fiscalQuarter(key) {
  ...
  const start = new Date(
    Date.UTC(fiscalStartYear, 6 + (quarterNumber - 1) * 3, 1),
  );
...
async function teamProfiles(client) {
  const profilesResult = await client
    .from('profiles')
    .select('email, full_name, sf_user_id, role');
...
function findBySfId(entries, sfUserId) {
  ...
  return (
    (entries || []).find(
      (entry) => sfIdKey(entry.sf_user_id || entry.Id || entry.id) === key,
    ) || null
  );
}
...
function uniqueOwnerIds(ids) {
  ...
  for (const id of ids || []) {
    ...
    if (!existing || String(id).length > String(existing).length)
      byKey.set(key, id);
  }
  return [...byKey.values()];
}
...
export function quarterForecast(signedToDate, records, fields) {
  const weightedOpen = records.reduce(
    (sum, record) =>
      sum +
      (number(record[fields.amount]) * number(record[fields.probability])) /
        100,
    0,
  );
...
function expectedRevenue(record, fields) {
  if (
    record[fields.expectedRevenue] !== null &&
    record[fields.expectedRevenue] !== undefined
  )
    return number(record[fields.expectedRevenue]);
  return (
    (number(record[fields.amount]) * number(record[fields.probability])) / 100
  );
}
...
export function buildCustomPipe(
  records,
  fields,
  today = dateKey(),
  ownerIds = null,
  ...
  const monthIndex = new Map(
    months.map((entry, index) => [entry.month, index]),
  );
  const byOwner = new Map();
  const opps = [];
  for (const record of records) {
    const owner = record[fields.ownerId];
    if (ownerIds && ![...ownerIds].some((id) => sfIdKey(id) === sfIdKey(owner)))
      continue;
    ...
    bucket.count += 1;
    const ownerBucket = bucket.by_owner[owner] || {
      amount: 0,
      expected: 0,
      count: 0,
    };
    ...
    const ownerRow = byOwner.get(owner) || {
      sf_user_id: owner,
      amount: 0,
      expected: 0,
      count: 0,
...
export function signedSeries(
  ...
  return starts.map((start) => {
    ...
    return quarterWon
      .filter(
        (record) =>
          sfIdKey(record[ownerField]) === sfIdKey(ownerId) &&
          dateKey(record[closeDateField]) <= endInclusive,
      )
...
function daysBetween(fromKey, toKey) {
  ...
  return Math.max(
    0,
    Math.round(
      (Date.parse(`${toKey}T12:00:00.000Z`) -
        Date.parse(`${fromKey}T12:00:00.000Z`)) /
        86400000,
    ),
  );
}
...
export function buildFollowUpOpps(records, fields, ownerIds, limit = 40) {
  ...
  return records
    ...
    .map((record) => ({
      ...
      close_date: record[fields.closeDate]
        ? dateKey(record[fields.closeDate])
        : null,
...
export function buildStagnantOpps(
  records,
  fields,
  ownerIds,
  today = dateKey(),
  ...
  for (const record of records) {
    ...
    if (
      quarterFrom &&
      quarterToExclusive &&
      (!close || close < quarterFrom || close >= quarterToExclusive)
    )
      continue;
    const amount = number(record[fields.amount]);
    if (amount <= 0) continue;
    const stageAnchor =
      record[fields.lastStageChangeDate] || record[fields.createdDate];
    const daysInStage = daysBetween(
      stageAnchor ? dateKey(stageAnchor) : null,
      today,
    );
    const activityAnchor = record[fields.lastActivityDate];
    const daysSinceActivity = activityAnchor
      ? daysBetween(dateKey(activityAnchor), today)
      : null;
    const threshold = stagnation[stage];
    const reasons = [];
    if (threshold && daysInStage !== null && daysInStage >= threshold)
      reasons.push('stage');
    if (daysSinceActivity === null || daysSinceActivity >= silentDays)
...
export function buildPace({
  ...
  const expected =
    expectedToDate !== null && expectedToDate !== undefined
      ? expectedToDate
      : linearExpected;
...
export function buildSeasonality(rows, asOf = dateKey()) {
  ...
  const yearTotal = Object.values(byMonth).reduce(
    (sum, value) => sum + value,
    0,
  );
  const month_of_year = {};
  ...
  for (const [qKey, months] of Object.entries(FISCAL_QUARTER_MONTHS)) {
    ...
    for (const month of months) {
      month_in_quarter[qKey][month] =
        qTotal > 0 ? (byMonth[month] || 0) / qTotal : 1 / months.length;
    }
  }
  return {
    as_of: asOf,
...
export function seasonalExpectedToDate(target, todayKey, quarter, seasonality) {
  if (target === null || target === undefined || !seasonality?.month_in_quarter)
    return null;
...
export function isArrWon(record, opportunity, saleTypeValues) {
  ...
  return (
    sale === saleTypeValues.catalogue[0] &&
    (opportunity.arrCommissionTypes || []).includes(commission)
  );
}
...
async function loadCachedSeasonality(client) {
  ...
  if (error) {
    if (
      /perf_seasonality_cache|schema cache|does not exist/i.test(
        error.message || '',
      )
    )
      return null;
...
async function upsertSeasonalityCache(
  client,
  { asOf, sampleFrom, sampleTo, payload },
) {
...
async function resolveSeasonality({
  client,
  accessToken,
  opportunity,
  seasonalityFrom,
  ...
  try {
    ...
    if (
      cached &&
      seasonalityCacheFresh(cached, today) &&
      cached.payload &&
...
async function priorSeriesFromSnapshots(
  client,
  priorWeekStarts,
  ownerIds,
  alignedStarts = priorWeekStarts,
) {
  ...
  const byKey = new Map(
    rows.map((row) => [`${sfIdKey(row.sf_user_id)}:${row.week_start}`, row]),
  );
  const priorPulse = [];
  const priorPipeline = [];
  for (const owner of ownerIds) {
    ...
    for (let index = 0; index < priorWeekStarts.length; index += 1) {
      ...
      priorPulse.push({
        ...
        call_results:
          snap?.call_results && typeof snap.call_results === 'object'
            ? snap.call_results
            : emptyCallResults(),
      });
      ...
      priorPipeline.push({
        ...
        closing_rate_amount: generatedAmount
          ? wonAmount / generatedAmount
          : null,
      });
    }
  }
...
async function loadBaselineStages(
  accessToken,
  opportunityHistory,
  oppIds,
  beforeDateTime,
  ...
  const chunks = [];
  for (let index = 0; index < oppIds.length; index += 200)
    chunks.push(oppIds.slice(index, index + 200));
  await Promise.all(
    chunks.map(async (chunk) => {
      ...
      const soql = query(
        ...
        [
          opportunityHistory.fields.opportunityId,
          opportunityHistory.fields.stageName,
          opportunityHistory.fields.createdDate,
        ],
...
async function loadForecastHistory(client, quarterLabel, ownerIds) {
  ...
  if (error) {
    ...
    if (
      /perf_forecast_snapshots|schema cache|does not exist/i.test(
        error.message || '',
      )
    )
      return [];
...
async function upsertForecastSnapshots(client, rows) {
  ...
  const { error } = await client
    .from('perf_forecast_snapshots')
    .upsert(rows, { onConflict: 'week_start,sf_user_id' });
  if (
    error &&
    !/perf_forecast_snapshots|schema cache|does not exist/i.test(
      error.message || '',
...
async function upsertWeekSnapshots(
  client,
  quarterLabel,
  rows,
  signedByOwner,
  ...
  const { error } = await client
    ...
    .upsert(payload, { onConflict: 'week_start,sf_user_id' });
  if (
    error &&
    !/perf_week_snapshots|schema cache|does not exist/i.test(
      error.message || '',
    )
  ) {
...
async function loadWeekSnapshotRows(client, weekStarts) {
  ...
  const { data, error } = await client
    .from('perf_week_snapshots')
    .select(
      'week_start, sf_user_id, iso_week, quarter, calls, meetings, proposals, progressions, call_results, generated_count, generated_amount, won_count, won_amount, won_catalogue, won_sur_mesure, won_conseil, won_arr_amount, signed_to_date, forecast',
    )
...
async function tryHistoricalSnapshotResponse({
  ...
  const [snapRows, profiles, targets, trackingOverrides, periodHistory] =
    await Promise.all([
  ...
  if (
    !teamView &&
    profile.sfUserId &&
    !ownerIds.some((id) => sfIdKey(id) === sfIdKey(profile.sfUserId))
  ) {
  ...
  const byKey = new Map(
    snapRows.map((row) => [
      `${sfIdKey(row.sf_user_id)}:${row.week_start}`,
      row,
    ]),
  );
  const pulse = [];
  ...
  const quarterRows = ownerIds.map((owner) => {
    ...
    const targetEntry = Object.entries(targets || {}).find(
      ([id]) => sfIdKey(id) === sfIdKey(owner),
    )?.[1];
    ...
    const expected =
      target === null
        ? null
        : target *
          ((weekIndex + 1) / Math.max(quarterStarts.length, weekIndex + 1));
    return {
      sf_user_id: owner,
  ...
  const forecastHistory = ownerIds.flatMap((owner) =>
    starts.map((start) => {
      const snap = byKey.get(`${sfIdKey(owner)}:${start}`);
      return {
        sf_user_id: owner,
  ...
  const paceTarget = quarterRows
    .map((row) => row.target)
    .filter((value) => value !== null);
  const targetSum = paceTarget.length
    ? paceTarget.reduce((sum, value) => sum + value, 0)
    : null;
  ...
  const compareWeekStart =
    window.starts.length > 1 ? window.starts[window.starts.length - 2] : null;
  const context = {
  ...
  return json(200, {
    ...
    owners: ownerIds.map((id) =>
      ownerMeta(
        ...
        [],
        id,
        id === profile.sfUserId || sfIdKey(id) === sfIdKey(profile.sfUserId)
    ...
    context,
    week_meta: starts.map((start) => ({
      week_start: start,
      iso_week: isoWeek(start),
    })),
    quarter_bounds: {
      from: quarter.from,
      to: addDays(quarter.toExclusive, -1),
      label: quarter.label,
    },
  });
}
...
function aggregateWeeklyRows({
  ...
  const row = (owner, start) => {
    ...
    if (!data.has(key)) {
      data.set(key, {
        ...
        won_count: 0,
        won_amount: 0,
        won_by_type: { catalogue: 0, sur_mesure: 0, conseil: 0 },
        won_arr_amount: 0,
        progressions: 0,
  ...
  for (const record of tasks)
    if (record[task.fields.subtype] === task.subtypeValue)
      inWindow(
        record,
        task.fields.ownerId,
  ...
  for (const record of events)
    inWindow(
      record,
      event.fields.ownerId,
      event.fields.activityDate,
      (target) => {
        target.meetings += 1;
      },
    );
  for (const record of histories)
    if (
      record[opportunityHistory.fields.stageName] ===
      opportunityHistory.stages.proposalSent
    )
      inWindow(
        record,
        opportunityHistory.fields.createdById,
        opportunityHistory.fields.createdDate,
        (target) => {
          target.proposals += 1;
        },
  ...
  for (const record of generated)
    inWindow(
      ...
      opportunity.fields.createdDate,
      (target) => {
        target.generated_count += 1;
        target.generated_amount += number(record[opportunity.fields.amount]);
      },
    );
  for (const record of won)
    inWindow(
      ...
      (target) => {
        ...
        const saleType = Object.entries(saleTypeValues).find(([, values]) =>
          values.includes(record[opportunity.saleTypeField]),
        )?.[0];
        target.won_count += 1;
        target.won_amount += amount;
        if (saleType) target.won_by_type[saleType] += amount;
        if (isArrWon(record, opportunity, saleTypeValues))
          target.won_arr_amount += amount;
      },
    );
  // wonArr conservé pour compat tests / appels historiques — préférer isArrWon sur `won`.
  for (const record of wonArr || [])
    inWindow(
      record,
      opportunity.fields.ownerId,
      opportunity.fields.closeDate,
      (target) => {
        target.won_arr_amount += number(record[opportunity.fields.amount]);
      },
    );
  ...
  for (const record of histories) {
    ...
    const start = mondayFor(
      dateKey(record[opportunityHistory.fields.createdDate]),
    );
    const progressionKey = `${opportunityId}:${start}`;
    if (
      previous &&
      opportunityHistory.stageOrder[stage] >
        opportunityHistory.stageOrder[previous] &&
      !progressed.has(progressionKey)
    ) {
      inWindow(
        record,
        opportunityHistory.fields.createdById,
...
export async function GET(request) {
  ...
  const period =
    rawPeriod === 'quarter' ? 'quarter' : rawPeriod === 'week' ? 'week' : null;
  ...
  const enrich =
    ...
    url.searchParams.get('parts') === 'enrich';
  // period=week → 2 semaines (S + S−1) ; period=quarter → trimestre fiscal ; weeks=N reste supporté pour tests.
  if (
    period === null &&
    (!Number.isInteger(weeks) || weeks < 1 || weeks > MAX_WEEKS)
  )
    return json(400, { error: 'invalid_weeks' });
  ...
  const today = dateKey();
  const effectiveDate =
    anchorWeekStart && /^\d{4}-\d{2}-\d{2}$/.test(anchorWeekStart)
  ...
  if (!teamView && !profile.sfUserId)
    return json(200, { ...empty, warning: 'sf_user_unmapped' });
  ...
  if (tokenResult.error || !tokenResult.accessToken)
    return json(502, { error: tokenResult.error || 'sf_auth_error' });
  const {
    task,
    event,
    opportunity,
  ...
  const openOppFields = [
    ...
    opportunity.fields.expectedRevenue,
    opportunity.fields.closeDate,
    opportunity.fields.createdDate,
    opportunity.fields.lastActivityDate,
    opportunity.fields.lastStageChangeDate,
    'Account.Name',
  ];
  ...
  if (teamView && profilesEarly === null)
    return json(500, { error: 'team_lookup_failed' });
  const scopeOwnerIds = uniqueOwnerIds(
    teamView
  ...
  const queryScopeIds = scopeOwnerIds.filter((id) => {
    ...
    return (
      Boolean(mapped) &&
      !isWeeklyOwnerExcluded(null, mapped.full_name || '', mapped.email || '')
    );
  });
  ...
  try {
    ...
    if (enrich) {
      const [openOpps, customOpen, seasonality, quarterWon, priorQuarterWon] =
        await Promise.all([
          crmRecords(
            ...
            query(
              opportunity.name,
              openOppFields,
              `${opportunity.fields.isClosed} = false AND ${opportunity.fields.stageName} != '${opportunityHistory.stages.stalledSuspect}'${byOwner(opportunity.fields.ownerId)}`,
            ),
          ),
          crmRecords(
            tokenResult.accessToken,
            query(
              ...
              `${opportunity.fields.isClosed} = false AND ${opportunity.saleTypeField} = '${escapeSOQL(saleTypeValues.sur_mesure[0])}' AND ${opportunity.fields.closeDate} >= ${today} AND ${opportunity.fields.closeDate} < ${customToExclusive}${byOwner(opportunity.fields.ownerId)}`,
            ),
          ),
          resolveSeasonality({
            client,
          ...
          crmRecords(
            ...
            query(
              ...
              `${opportunity.fields.isWon} = true AND ${opportunity.fields.closeDate} >= ${quarter.from} AND ${opportunity.fields.closeDate} < ${quarter.toExclusive}${byOwner(opportunity.fields.ownerId)}`,
            ),
          ),
          crmRecords(
            tokenResult.accessToken,
            query(
              ...
              `${opportunity.fields.isWon} = true AND ${opportunity.fields.closeDate} >= ${priorQuarter.from} AND ${opportunity.fields.closeDate} < ${priorQuarter.toExclusive}${byOwner(opportunity.fields.ownerId)}`,
            ),
          ),
        ]);
      ...
      const owners = new Set(
        teamView ? [] : profile.sfUserId ? [profile.sfUserId] : [],
      );
      const addOwners = (records, field) =>
        records.forEach((record) => {
          if (record[field]) owners.add(record[field]);
        });
      ...
      for (const record of openOpps) {
        ...
        if (
          allowed(owner) &&
          !record[opportunity.fields.isClosed] &&
          record[opportunity.fields.stageName] !==
            opportunityHistory.stages.stalledSuspect
        ) {
          openByOwner.set(owner, (openByOwner.get(owner) || 0) + 1);
        }
      }
      const quarterOpen = quarterOpenFromOpenOpps(
        openOpps,
        opportunity.fields,
        quarter,
      );
      const customPipe = buildCustomPipe(
        customOpen,
        opportunity.fields,
        effectiveDate,
        owners,
      );
      const followUpOpps = buildFollowUpOpps(
        quarterOpen,
        opportunity.fields,
        owners,
      );
      const stagnantOpps = buildStagnantOpps(
        openOpps,
        opportunity.fields,
        owners,
        effectiveDate,
      ...
      if (resolvedPeriod === 'quarter' && priorStarts.length) {
        const priorWeekStarts = priorStarts.slice(
          0,
          Math.min(weekIndex + 1, priorStarts.length),
        );
        ...
        const fromSnaps = await priorSeriesFromSnapshots(
          client,
          priorWeekStarts,
          ownerIds,
          alignedStarts,
      ...
      for (const record of quarterWon) {
        ...
        if (allowed(owner))
          signedByOwner.set(
            owner,
            (signedByOwner.get(owner) || 0) +
              number(record[opportunity.fields.amount]),
          );
      }
      for (const record of quarterOpen) {
        const owner = record[opportunity.fields.ownerId];
        if (allowed(owner))
          openQuarterByOwner.set(owner, [
            ...(openQuarterByOwner.get(owner) || []),
            record,
          ]);
      }
      for (const record of customOpen) {
        ...
        if (allowed(owner))
          customByOwner.set(
            owner,
            (customByOwner.get(owner) || 0) +
              number(record[opportunity.fields.amount]),
          );
      }
      const quarterRows = ownerIds.map((owner) => {
        const signedToDate = signedByOwner.get(owner) || 0;
        const { weightedOpen, forecast } = quarterForecast(
          signedToDate,
          openQuarterByOwner.get(owner) || [],
          opportunity.fields,
        );
        ...
        const signedN1 =
          priorSignedValues[
            Math.min(weekIndex, Math.max(0, priorSignedValues.length - 1))
          ] ?? 0;
        const seasonalExpected = seasonalExpectedToDate(
          target,
          effectiveDate,
          quarter,
          seasonality,
        ...
        return {
          ...
          pace_ratio: expected && expected > 0 ? signedToDate / expected : null,
          expected_to_date: expected,
          monthly_indicative: target
            ? quarterlyToMonthlyIndicative(target, quarter.label, seasonality)
            : [],
        };
      });
      const paceTargets = quarterRows
        .map((row) => row.target)
        .filter((value) => value !== null);
      const paceTarget = paceTargets.length
        ? paceTargets.reduce((sum, value) => sum + value, 0)
        : null;
      const seasonalExpected = seasonalExpectedToDate(
        paceTarget,
        effectiveDate,
      ...
      const pace = buildPace({
        ...
        forecast: quarterRows.reduce((sum, row) => sum + row.forecast, 0),
        target: paceTarget,
        signedN1: quarterRows.reduce(
          (sum, row) => sum + (row.signed_n1 || 0),
          0,
        ),
        weekOfQuarter: weekIndex + 1,
        weeksInQuarter: Math.max(quarterStarts.length, weekIndex + 1),
        wonCount: quarterWon.filter((record) =>
          allowed(record[opportunity.fields.ownerId]),
        ).length,
    ...
    const [
      ...
      crmRecords(
        ...
        query(
          ...
          `${task.fields.subtype} = '${task.subtypeValue}' AND ${task.fields.activityDate} >= ${fromDate} AND ${task.fields.activityDate} < ${toDate}${byOwner(task.fields.ownerId)}`,
        ),
      ),
      crmRecords(
        tokenResult.accessToken,
        query(
          event.name,
          [event.fields.ownerId, event.fields.activityDate],
          `${event.fields.activityDate} >= ${fromDate} AND ${event.fields.activityDate} < ${toDate}${byOwner(event.fields.ownerId)}`,
        ),
      ),
      crmRecords(
        tokenResult.accessToken,
        query(
          ...
          `${opportunityHistory.fields.createdDate} >= ${fromDateTime} AND ${opportunityHistory.fields.createdDate} < ${toDateTime}${byOwner(opportunityHistory.fields.createdById)}`,
      ...
      crmRecords(
        ...
        query(
          ...
          `${opportunity.fields.createdDate} >= ${fromDateTime} AND ${opportunity.fields.createdDate} < ${toDateTime}${byOwner(opportunity.fields.ownerId)}`,
        ),
      ),
      ...
      crmRecords(
        ...
        query(
          ...
          [
            ...
            opportunity.fields.amount,
            opportunity.saleTypeField,
            opportunity.commissionTypeField,
          ],
          `${opportunity.fields.isWon} = true AND ${opportunity.fields.closeDate} >= ${fromDate} AND ${opportunity.fields.closeDate} < ${toDate}${byOwner(opportunity.fields.ownerId)}`,
        ),
      ),
      needBoard
        ? crmRecords(
      ...
      lite
        ? crmRecords(
            ...
            query(
              opportunity.name,
              quarterOpenFields,
              `${opportunity.fields.isClosed} = false AND ${opportunity.fields.closeDate} >= ${quarter.from} AND ${opportunity.fields.closeDate} < ${quarter.toExclusive}${byOwner(opportunity.fields.ownerId)}`,
            ),
          )
        : Promise.resolve([]),
      needBoard
    ...
    const wonArr = [];
    const quarterOpen = needBoard
      ? quarterOpenFromOpenOpps(openOpps, opportunity.fields, quarter)
      : quarterOpenLite;
    const profiles = profilesEarly || [];
    const targets = targetsEarly || {};
    ...
    const windowOppIds = [
      ...new Set(
        histories
          .map((record) => record[opportunityHistory.fields.opportunityId])
          .filter(Boolean),
      ),
    ];
    let baselineStages = new Map();
    try {
      baselineStages = await loadBaselineStages(
        tokenResult.accessToken,
        opportunityHistory,
        windowOppIds,
        fromDateTime,
      ...
      sfUsers = candidateOwnerIds.length
        ? await crmRecords(
            ...
            query(
              sfUserObject.name,
              [
                sfUserObject.fields.id,
                sfUserObject.fields.name,
                sfUserObject.fields.email,
                sfUserObject.fields.isActive,
              ],
    ...
    const activeOwnerIds = candidateOwnerIds.filter((id) => {
      ...
      return !isWeeklyOwnerExcluded(
        sfUser,
        mapped?.full_name || sfUser?.Name || '',
        mapped?.email || sfUser?.Email || '',
      );
    });
    if (
      !teamView &&
      profile.sfUserId &&
      !activeOwnerIds.some((id) => sfIdKey(id) === sfIdKey(profile.sfUserId))
    ) {
    ...
    const customPipe = lite
      ? empty.custom_pipe
      : buildCustomPipe(customOpen, opportunity.fields, effectiveDate, owners);
    const followUpOpps = lite
      ? []
      : buildFollowUpOpps(quarterOpen, opportunity.fields, owners);
    const stagnantOpps = lite
      ? []
      : buildStagnantOpps(
          openOpps,
          opportunity.fields,
    ...
    if (!lite && resolvedPeriod === 'quarter' && priorStarts.length) {
      ...
      if (priorWeekStarts.length) {
        ...
        if (fromSnaps) {
          ...
          const [pTasks, pEvents, pHistories, pGenerated, pWon] =
            await Promise.all([
              crmRecords(
                tokenResult.accessToken,
                query(
                  ...
                  `${task.fields.subtype} = '${task.subtypeValue}' AND ${task.fields.activityDate} >= ${pFrom} AND ${task.fields.activityDate} < ${pToEx}${byOwner(task.fields.ownerId)}`,
                ),
              ),
              crmRecords(
                tokenResult.accessToken,
                query(
                  event.name,
                  [event.fields.ownerId, event.fields.activityDate],
                  `${event.fields.activityDate} >= ${pFrom} AND ${event.fields.activityDate} < ${pToEx}${byOwner(event.fields.ownerId)}`,
                ),
              ),
              crmRecords(
                tokenResult.accessToken,
                query(
                  ...
                  `${opportunityHistory.fields.createdDate} >= ${pFromDt} AND ${opportunityHistory.fields.createdDate} < ${pToDt}${byOwner(opportunityHistory.fields.createdById)}`,
                  `${opportunityHistory.fields.opportunityId}, ${opportunityHistory.fields.createdDate}`,
                ),
              ),
              crmRecords(
                ...
                query(
                  ...
                  `${opportunity.fields.createdDate} >= ${pFromDt} AND ${opportunity.fields.createdDate} < ${pToDt}${byOwner(opportunity.fields.ownerId)}`,
                ),
              ),
              crmRecords(
                tokenResult.accessToken,
                query(
                  ...
                  `${opportunity.fields.isWon} = true AND ${opportunity.fields.closeDate} >= ${pFrom} AND ${opportunity.fields.closeDate} < ${pToEx}${byOwner(opportunity.fields.ownerId)}`,
                ),
              ),
            ]);
          ...
          priorPulse = priorRows.map((entry) => {
            ...
            const alignedStart =
              startIndex >= 0 ? alignedStarts[startIndex] : entry.week_start;
            return {
          ...
          priorPipeline = priorRows.map((entry) => {
            ...
            return {
              ...
              closing_rate_count: entry.generated_count
                ? entry.won_count / entry.generated_count
                : null,
              closing_rate_amount: entry.generated_amount
                ? entry.won_amount / entry.generated_amount
                : null,
            };
          });
        }
    ...
    const quarterRows = ownerIds.map((owner) => {
      ...
      const expected = seasonalExpected ?? linearExpected;
      const pace_ratio =
        expected && expected > 0 ? signedToDate / expected : null;
      return {
        sf_user_id: owner,
        quarter: quarter.label,
        signed_to_date: signedToDate,
    ...
    if (isLive) {
      const forecastByOwner = new Map(
        quarterRows.map((entry) => [entry.sf_user_id, entry.forecast]),
      );
      void Promise.all([
        upsertForecastSnapshots(
          client,
          quarterRows.map((entry) => ({
    ...
    const historyByOwnerWeek = new Map(
      storedHistory.map((entry) => [
        `${sfIdKey(entry.sf_user_id)}:${entry.week_start}`,
        entry,
      ]),
    );
    const signedByOwnerWeek = new Map();
    for (const owner of ownerIds) {
      signedByOwnerWeek.set(
        owner,
        signedSeries(
          window.starts,
          quarterWon,
      ...
      window.starts.map((start, index) => {
        ...
        return {
          ...
          forecast: isCurrent
            ? (live?.forecast ?? null)
            : stored
              ? number(stored.forecast)
              : null,
          signed_to_date: isCurrent
            ? (live?.signed_to_date ?? signedValues[index])
            : stored
              ? number(stored.signed_to_date)
              : signedValues[index],
        };
      }),
    );
    ...
    const paceWithMonthly = {
      ...pace,
      monthly_indicative: paceTarget
        ? quarterlyToMonthlyIndicative(paceTarget, quarter.label, seasonality)
        : [],
    };
    ...
    return json(200, {
      ...
      pulse: rows.map(
        ({
          ...
          meetings,
          proposals,
          call_results: call_results || emptyCallResults(),
        }),
      ),
      pipeline: rows.map((entry) => ({
        ...(({
          sf_user_id,
          week,
          week_start,
      ...
      prior_pulse: priorPulse,
      prior_pipeline: priorPipeline,
      effort: rows.map((entry) => {
        const open = openByOwner.get(entry.sf_user_id) || 0;
        return {
          sf_user_id: entry.sf_user_id,
          week: entry.week,
          ...
          effort_rate: open ? entry.progressions / open : null,
        };
      }),
      quarter: quarterRows,
      forecast_history: forecastHistory,
      ...
      period_history: periodHistory,
      context,
      week_meta: window.starts.map((start) => ({
        week_start: start,
        iso_week: isoWeek(start),
      })),
      quarter_bounds: {
