...
const { mockVerifyJWT, mockGetProfile, mockFetchSFToken, mockSearchContacts } =
  vi.hoisted(() => ({
    mockVerifyJWT: vi.fn(),
...
vi.mock('./_crm/salesforce.js', () => ({
  ...
  buildLightningUrl: (type, id) =>
    id ? `https://sf.test/lightning/r/${type}/${id}/view` : null,
}));
...
function request(query = '') {
  return new Request(`https://xos.test/api/perf${query}`, {
...
function recordSet() {
  return {
    tasks: [
      { OwnerId: '005A', ActivityDate: '2026-07-07', TaskSubtype: 'Call' },
      { OwnerId: '005A', ActivityDate: '2026-07-07', TaskSubtype: 'Email' },
    ],
    events: [{ OwnerId: '005A', ActivityDate: '2026-07-08' }],
    histories: [
      {
        OpportunityId: 'opp-1',
        StageName: 'Projet identifié',
        CreatedDate: '2026-06-30T09:00:00.000Z',
        CreatedById: '005A',
      },
      {
        OpportunityId: 'opp-1',
        StageName: 'Proposition envoyée',
        CreatedDate: '2026-07-07T09:00:00.000Z',
        CreatedById: '005A',
      },
      {
        OpportunityId: 'opp-1',
        StageName: 'XOS short-listé',
        CreatedDate: '2026-07-08T09:00:00.000Z',
        CreatedById: '005A',
      },
    ],
    generated: [
      {
        OwnerId: '005A',
        CreatedDate: '2026-07-09T09:00:00.000Z',
        CloseDate: '2026-07-10',
        IsWon: true,
        IsClosed: true,
        StageName: 'Fermée / Gagnée',
        Amount: 100,
      },
    ],
    won: [
      {
        ...
        Type_de_commission__c: 'Abonnement 3 ans',
      },
      {
        OwnerId: '005A',
        CreatedDate: '2026-06-01T09:00:00.000Z',
        ...
        StageName: 'Fermée / Gagnée',
        Amount: 30,
        Type_de_vente__c: 'LMS',
        Type_de_commission__c: null,
      },
      {
        OwnerId: '005A',
        CreatedDate: '2026-06-01T09:00:00.000Z',
    ...
    openOpps: [
      {
        ...
        LastStageChangeDate: '2026-04-15',
        'Account.Name': 'Acme',
      },
      {
        OwnerId: '005A',
        IsClosed: false,
        StageName: 'Suspect enlisé',
        Amount: 5000,
        Probability: 10,
      },
      {
...
function queueSalesforce(records = recordSet(), baseline = []) {
  ...
  mockSearchContacts.mockImplementation(async (_token, soql) => {
    if (
      soql.includes('FROM OpportunityHistory') &&
      soql.includes('OpportunityId IN')
    )
      return { records: baseline };
    if (soql.includes('FROM OpportunityHistory'))
      return { records: records.histories };
    ...
    if (soql.includes('CALENDAR_YEAR') || soql.includes('CALENDAR_MONTH'))
      return { records: [] };
    if (soql.includes('TaskSubtype') || /\bFROM Task\b/.test(soql))
      return { records: records.tasks };
    if (/\bFROM Event\b/.test(soql)) return { records: records.events };
    if (
      soql.includes('LastActivityDate') ||
      soql.includes('LastStageChangeDate')
    )
      return { records: records.openOpps };
    if (soql.includes('Sur-mesure') || soql.includes('sur_mesure'))
      return { records: records.customOpen };
    if (soql.includes('IsWon = true') && soql.includes('2025-07-01'))
      return { records: records.priorWon };
    if (
      soql.includes('IsWon = true') &&
      soql.includes('2026-07-01') &&
      !soql.includes('CreatedDate')
    )
      return { records: records.quarterWon };
    if (soql.includes('IsWon = true')) return { records: records.won };
    if (
      soql.includes('IsClosed = false') &&
      soql.includes('CloseDate >=') &&
      soql.includes('2026-07-01')
    )
      return { records: records.quarterOpen };
...
beforeEach(() => {
  ...
  mockFrom.mockImplementation((table) => {
    if (table === 'profiles')
      return {
        select: () => Promise.resolve({ data: teamProfiles, error: null }),
      };
...
describe('fiscal quarter helpers', () => {
  it('computes the January quarter in the fiscal year named by its ending year', () => {
    expect(perf.fiscalQuarter('2026-01-15')).toEqual({
      from: '2026-01-01',
  ...
  it('computes the August quarter across the July fiscal-year boundary', () => {
    expect(perf.fiscalQuarter('2026-08-20')).toEqual({
      from: '2026-07-01',
  ...
  it('adds signed and probability-weighted open amounts', () => {
    expect(
      perf.quarterForecast(
        100,
        [
          { Amount: 200, Probability: 50 },
          { Amount: 100, Probability: 25 },
        ],
        { amount: 'Amount', probability: 'Probability' },
      ),
    ).toEqual({ weightedOpen: 125, forecast: 225 });
  });

  it('builds prior fiscal quarter one year earlier', () => {
    expect(perf.priorFiscalQuarter('2026-07-11')).toEqual({
      from: '2025-07-01',
  ...
  it('builds seasonality weights for year and month-in-quarter', () => {
    ...
    const expected = perf.seasonalExpectedToDate(
      30000,
      '2026-07-15',
      { label: 'FY27-Q1' },
      seasonality,
    ...
    const rows = perf.buildFollowUpOpps(
      [
        ...
        },
        {
          ...
          Name: 'B',
          OwnerId: '005A',
          StageName: 'Négo financière engagée',
          Amount: 8000,
          Probability: 50,
      ...
      [
        {
          ...
          LastStageChangeDate: '2026-06-20',
        },
        {
          Id: '2',
          Name: 'Fresh',
          ...
          LastStageChangeDate: '2026-07-08',
        },
      ],
      fields,
      ['005A'],
...
describe('GET /api/perf', () => {
  ...
  it('aggregates the W28 pilot week by owner using the frozen metric definitions', async () => {
    ...
    expect(body.pulse.find((row) => row.week === week)).toMatchObject({
      sf_user_id: '005A',
      calls: 1,
      meetings: 1,
      proposals: 1,
    });
    expect(body.pipeline.find((row) => row.week === week)).toMatchObject({
      generated_count: 1,
      generated_amount: 100,
      won_count: 3,
      won_amount: 100,
      closing_rate_count: 3,
      closing_rate_amount: 1,
    });
    expect(body.effort.find((row) => row.week === week)).toMatchObject({
      progressions: 1,
      open_opps_at_start: 3,
      effort_rate: 1 / 3,
    });
  });
  ...
  it('returns the team roster to a commercial for the Moi/Équipe toggle', async () => {
    ...
    expect(body.view).toBe('team');
    expect(body.owners.map((owner) => owner.sf_user_id)).toEqual([
      '005A',
      '005B',
    ]);
  ...
  it('includes sf_user_map members in the commercial team roster', async () => {
    mockFrom.mockImplementation((table) => {
      if (table === 'profiles') {
        return {
          select: () =>
            Promise.resolve({ data: [teamProfiles[0]], error: null }),
        };
      }
      if (table === 'sf_user_map') {
        return {
          select: () =>
            Promise.resolve({
              data: [
                { email: 'paul.rathouin@xos-learning.fr', sf_user_id: '005C' },
              ],
              error: null,
            }),
        };
    ...
    expect(body.view).toBe('team');
    expect(
      body.owners.map((owner) => owner.sf_user_id.slice(0, 15)).sort(),
    ).toEqual(['005A', '005C']);
    expect(
      body.pulse.some((row) => row.sf_user_id.slice(0, 15) === '005C'),
    ).toBe(true);
  });
  ...
  it('loads won deals once and derives ARR in JS from mapped commission types', async () => {
    await GET(request());
    const queries = mockSearchContacts.mock.calls.map(([, soql]) => soql);
    const wonQueries = queries.filter(
      (soql) =>
        soql.includes('IsWon = true') &&
        soql.includes(mapping.objects.opportunity.commissionTypeField),
    );
    expect(wonQueries.length).toBeGreaterThan(0);
    expect(
      wonQueries.some((soql) =>
        mapping.objects.opportunity.arrCommissionTypes.every((value) =>
          soql.includes(`'${value}'`),
        ),
      ),
    ).toBe(false);
    expect(wonQueries[0]).toContain(mapping.objects.opportunity.saleTypeField);
    expect(wonQueries[0]).toContain(
      mapping.objects.opportunity.commissionTypeField,
    );
  });

  it('aggregates tracked sale types without bucketing LMS or XOS+, plus ARR and quarter metrics', async () => {
    const body = await (await GET(request())).json();
    expect(Object.keys(mapping.objects.opportunity.saleTypes)).toEqual([
      'catalogue',
      'sur_mesure',
      'conseil',
    ]);
    ...
    expect(body.follow_up_opps[0]).toMatchObject({
      id: '006OPEN',
      expected: 4800,
    });
    expect(body.stagnant_opps.some((row) => row.id === '006OPEN')).toBe(true);
    expect(body.pace).toMatchObject({
      week_of_quarter: 1,
      signed_to_date: 100,
      signed_n1: 80,
      target: 60000,
    ...
    expect(body.seasonality).toMatchObject({
      month_of_year: expect.any(Object),
      month_in_quarter: expect.any(Object),
    });
    expect(
      body.pulse.find((row) => row.week === '2026-W28')?.call_results,
    ).toEqual(expect.any(Object));

    const quarterBody = await (await GET(request('?period=quarter'))).json();
    expect(quarterBody.seasonality).toMatchObject({
      month_of_year: expect.any(Object),
      month_in_quarter: expect.any(Object),
    });
  });
  ...
  it('counts the first in-window transition thanks to the pre-window baseline', async () => {
    ...
    queueSalesforce(records, [
      { OpportunityId: 'opp-9', StageName: 'Projet identifié' },
    ]);

    const body = await (await GET(request('?weeks=8'))).json();
    const baselineQuery = mockSearchContacts.mock.calls
      .map(([, soql]) => soql)
      .find((soql) => soql.includes("OpportunityId IN ('opp-9')"));
    expect(baselineQuery).toContain('CreatedDate < 2026-05-18T00:00:00Z');
    expect(body.effort.find((row) => row.week === '2026-W28')).toMatchObject({
      progressions: 1,
    });
  });
  ...
  it('returns forecast history points for the effort chart', async () => {
    ...
    const ada = body.forecast_history.find(
      (row) => row.sf_user_id === '005A' && row.week === '2026-W28',
    );
  ...
  it('returns custom_pipe monthly buckets with expected revenue', async () => {
    ...
    expect(
      body.custom_pipe.months.find((month) => month.month === '2026-10'),
    ).toMatchObject({ amount: 300, expected: 150, count: 1 });
    expect(body.custom_pipe.opps[0]).toMatchObject({
      name: 'Deal SM',
      expected: 150,
    });
  });
  ...
  it('excludes inactive former commercials from the team roster', async () => {
      ...
      if (soql.includes('FROM User')) {
        return {
          records: [
            ...
            {
              Id: '005R',
              Name: 'Romain Waeselynck',
              Email: 'romain@xos-learning.fr',
              IsActive: false,
            },
          ],
        };
    ...
    const body = await (await GET(request('?weeks=8'))).json();
    expect(body.owners.map((owner) => owner.sf_user_id).sort()).toEqual([
      '005A',
      '005B',
    ]);
    expect(body.owners.some((owner) => /waeselynck/i.test(owner.name))).toBe(
      false,
    );
  });
  ...
  it('discovers SF owners when profiles only has an excluded admin', async () => {
          ...
          select: () => ({
            eq: (_col, key) => ({
              maybeSingle: () =>
                Promise.resolve({
                  data:
                    key === 'weekly_targets' ? { value: {} } : { value: {} },
                  error: null,
                }),
            }),
          }),
    ...
    expect(body.owners.some((owner) => /th[eé]o/i.test(owner.name))).toBe(
      false,
    );
    ...
    expect(
      soql.some(
        (q) =>
          /OwnerId IN \('005T'\)/.test(q) ||
          /CreatedById IN \('005T'\)/.test(q),
      ),
    ).toBe(false);
  });
  ...
  it('lite mode skips heavy open-opp queries and reports timing', async () => {
    ...
    expect(
      soql.some((q) => /LastActivityDate|LastStageChangeDate/.test(q)),
    ).toBe(false);
  ...
  it('enrich returns board payload without replaying the week pulse queries', async () => {
    ...
    expect(
      soql.some(
        (q) => q.includes('OpportunityHistory') && q.includes('CreatedDate >='),
      ),
    ).toBe(false);
  });
  ...
  it('aligns prior_pulse and prior_pipeline week_start on the current quarter', async () => {
    ...
    expect(body.prior_pulse[0]).toMatchObject({
      sf_user_id: '005A',
      week_start: '2026-07-06',
    });
    expect(body.prior_pipeline[0]).toMatchObject({
      sf_user_id: '005A',
      week_start: '2026-07-06',
    });
    expect(
      body.prior_pulse.every((row) => quarterStarts.includes(row.week_start)),
    ).toBe(true);
    expect(
      body.prior_pipeline.every((row) =>
        quarterStarts.includes(row.week_start),
      ),
    ).toBe(true);
  ...
  it('ignores sur-mesure records that fall outside the 6 monthly buckets', () => {
    ...
    expect(result.by_owner).toEqual([
      { sf_user_id: '005A', amount: 300, expected: 150, count: 1 },
    ]);
    expect(result.opps).toHaveLength(1);
    const monthsFlat = result.months.flatMap((m) => [
      m.amount,
      m.expected,
      m.count,
    ]);
