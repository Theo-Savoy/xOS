...
const {
  mockVerifyJWT,
  mockStartSalesforceOAuth,
  mockCompleteSalesforceOAuth,
  mockStoreSalesforceRefreshToken,
...
describe('GET /api/auth', () => {
  it('redirects Salesforce flow to the login screen with sf_coming_soon until OAuth is wired', async () => {
    const response = await GET(
      new Request('https://xos.hellotheo.fr/api/auth?flow=salesforce'),
    );
  ...
  it('rejects unrecognized flows', async () => {
    const response = await GET(
      new Request('https://xos.hellotheo.fr/api/auth'),
    );
  ...
  it('completes the dedicated Salesforce link callback', async () => {
    ...
    expect(response.status).toBe(302);
    expect(response.headers.get('Location')).toBe(
      'https://xos.hellotheo.fr/?sf_link=success',
    );
  });
});
...
describe('POST /api/auth', () => {
  beforeEach(() => {
    ...
    mockGetServiceClient.mockReturnValue({ from: vi.fn() });
    mockStartSalesforceOAuth.mockResolvedValue({
      authorizationUrl: 'https://login.salesforce.test/authorize',
    });
  ...
  it('returns 204 without any session cookie after JWT verification', async () => {
    const response = await POST(
      new Request('https://xos.hellotheo.fr/api/auth', { method: 'POST' }),
    );
  ...
  it('starts account linking only for an authenticated X OS user', async () => {
    const response = await POST(
      new Request('https://xos.hellotheo.fr/api/auth?flow=salesforce-link', {
        method: 'POST',
    ...
    await expect(response.json()).resolves.toEqual({
      authorization_url: 'https://login.salesforce.test/authorize',
    });
    expect(mockStartSalesforceOAuth).toHaveBeenCalledWith(
  ...
  it('automatically stores the Salesforce provider refresh token during the session bridge', async () => {
    ...
    expect(response.status).toBe(204);
    expect(mockStoreSalesforceRefreshToken).toHaveBeenCalledWith(
      expect.objectContaining({
        user: { id: 'user-1' },
