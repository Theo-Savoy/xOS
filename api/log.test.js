...
describe('POST /api/launcher', () => {
  beforeEach(async () => {
    ...
    mockMaybeSingleProfile.mockResolvedValue({
      data: {
        sf_refresh_token_encrypted: USER_REFRESH_CIPHERTEXT,
        sf_user_id: '005AA0000012345AAA',
  ...
  describe('action: log_call', () => {
    ...
    it('attribue la Task au commercial quand profiles.sf_user_id est mappé', async () => {
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ access_token: 'sf-access-token' }), {
          status: 200,
        }),
      );
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'task-sf-id-456', success: true }), {
          status: 201,
        }),
      );
    ...
    it('creates a Salesforce Task with WhatId and logs to database on success', async () => {
      ...
      // 2. SF Task Creation
      fetchSpy.mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'task-sf-id-123', success: true }), {
          status: 201,
        }),
      );
  ...
  describe('action: create_contact', () => {
    ...
    it('returns 400 invalid_phone when phone is not a string', async () => {
      const res = await POST(
        makeReq({ action: 'create_contact', lastName: 'Dupont', phone: 12345 }),
      );
      expect(res.status).toBe(400);
      const body = await res.json();
    ...
    it('returns 400 invalid_email when email is not a string (no coercion)', async () => {
      const res = await POST(
        makeReq({ action: 'create_contact', lastName: 'Dupont', email: 42 }),
      );
      expect(res.status).toBe(400);
      const body = await res.json();
      expect(body.error).toBe('invalid_email');
    ...
    it('omits whitespace-only optional fields from SF payload (empty strings trimmed away)', async () => {
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({ id: 'contact-sf-id-999', success: true }),
          { status: 201 },
        ),
      );
    ...
    it('creates a Salesforce Contact with trimmed strings and logs to database on success', async () => {
      ...
      fetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({ id: 'contact-sf-id-789', success: true }),
          { status: 201 },
        ),
      );
