...
Compagnon opérationnel de `xos_portal_plan.md` (v2). Découpage en **lots délégables**, un lot = un agent = un worktree Orca = une PR.
...
- Google SSO restreint au domaine ; trigger de création de `profiles` à la première connexion.
...
- **Vérifié par** : login Google fonctionnel en prod, profil créé en base, endpoint de test refusant les requêtes sans JWT, `/dashboard.html` accessible dans les deux modes d'auth.
...
|---|---|---|---|
| 0 | 0.1 → 0.2 | séquentiel | Socle déployé, auth Google OK |
...
- ~~Domaine(s) Google Workspace autorisés pour le SSO~~ → **`xos-learning.fr`** (fourni le 2026-07-10).
- **Création du CNAME `xos.hellotheo.fr`** + ajout du domaine au projet Vercel → nécessaire au plus tard pour le jalon V1.
