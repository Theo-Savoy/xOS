...
- **Lot v2.A — Adapter CRM + mapping + moteur SOQL** — `Command Code` : migrations `005_crm_mapping` / `006_call_target_presets`, `api/crm/salesforce.js` (adapter), `api/calls-list.js` réécrit (arbre de filtres entreprise/contact/relance, OU intra-famille, dédup, NPA).
