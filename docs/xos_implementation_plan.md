...
4. **Transition Labo** : avant le lot 10.7, `GET /dashboard.html`, `GET /api/refresh` et `POST /api/update` restent fonctionnels. Au lot 10.7, ce contrôle est remplacé par la matrice de parité native complète, la preuve d'import Supabase et un scan à zéro des usages runtime legacy.
...
- `dashboard.html`, `api/refresh.py`, `api/update.js`, `api/history.js` restent **intouchables hors Phase 10**. Leur retrait n'est permis que dans le lot final 10.7 après les gates de parité et migration.
...
### Lot 1.3 — App Labo, alors nommée CRM Cleaner (iframe) — ✅
- `src/apps/cleaner/` : fenêtre contenant l'iframe `/dashboard.html`, taille par défaut adaptée, entrée dock.
- **Vérifié par** : parcours complet du Labo legacy *dans* la fenêtre X OS (filtres, tri, action en lot de test) identique à l'accès direct.

**🎯 Jalon V1 déployée** : bureau + dock + Labo legacy utilisable. Annonce possible à l'équipe.
...
## Phase 3 — Weekly Perf — ✅ *(3.0 → 3.3 livrés — dernière livraison 2026-07-11 nuit)*
...
### Lot 3.3 — Rituel équipe (remplace le Google Sheet) — ✅ *(livré 2026-07-11 nuit — Codex Terra medium : breakdown type de vente + ARR + bloc trimestre fiscal + vue Tableau avec Total/Moyenne, 414 tests ; « ventes exceptionnelles » retirée sur décision Théo)*
...
- **Lot 8.1b — Attribution niveau 2 (écritures sous l'identité du commercial)** — 🟡 *(code prêt, activation prod à valider)* : Call Manager, Launcher et Labo utilisent le refresh token SF **du commercial** quand il existe, avec fallback intégration. Cache 30 min par user + retry 401 sous la même identité ; le legacy de Labo exige désormais le JWT X OS et le reçoit par `postMessage` same-origin. **Reste** : lier un compte test et vérifier `CreatedById` en live.
...
## Phase 10 — Labo *(reconstruction modulaire — priorité courante)* — ⬜
...
- Remplacement de l'iframe dans `src/apps/cleaner/` par un shell modulaire : Accueil fixe, un onglet par module, état conservé.
...
- Scanner les usages runtime ; seulement après zéro appel legacy : retirer `public/dashboard.html`, `api/refresh.py`, `api/update.js`, `api/history.js`, `api/version.js` et les règles middleware associées.
