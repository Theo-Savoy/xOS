...
- [ ] **Step 1: Écrire les builders de fixture et un harness de fetch minimal.**
...
- [ ] **Step 2: Écrire les tests rouges des comportements attendus et défauts connus.**

  Les tests couvriront notamment : queue FIFO après deux soumissions rapides, restauration du snapshot sur rejet `log_call`, claim unique avec succès puis claim concurrent `contact_claimed`, contexte courant + trois suivants préchargés, transaction `log_call` puis `log_event`, vagues de 4 avec échec partiel, presets rapides/date et NPA, contact partagé exclu du pool et affiché comme pris, raccourcis ignorés dans les champs et pendant le bulk, et garde `L` pendant conversation Power. Les deux défauts explicitement listés par le contrat (`L` capturé avant la conversation et runner `aria-hidden` réactif sous pré-session) seront marqués `it.fails` avec un commentaire expliquant la migration qui doit les faire passer.

- [ ] **Step 3: Exécuter uniquement les nouveaux tests pour confirmer les rouges légitimes.**
  ...
  Expected: les assertions de comportement legacy passent ; seuls les deux gardes de défauts connus sont des échecs attendus via `it.fails`.

- [ ] **Step 4: Stabiliser les fixtures sans production code.**
...
- [ ] **Step 5: Exécuter le sous-ensemble vert et vérifier les fichiers chauds.**
...
- [ ] **Step 6: Committer le livrable de caractérisation métier.**
...
- [ ] **Step 1: Écrire les assertions axe-like et live regions.**
...
- [ ] **Step 2: Écrire le test reduced-motion.**
...
- [ ] **Step 3: Écrire le garde pré-session.**
...
- [ ] **Step 4: Exécuter le sous-ensemble a11y.**
...
- [ ] **Step 5: Committer le livrable a11y.**
...
- [ ] **Step 1: Écrire la fixture DOM déterministe et ses états.**
...
- [ ] **Step 2: Écrire le test responsive rouge.**
...
- [ ] **Step 3: Générer les captures avec le même environnement.**

  Run: `npx playwright test e2e/runner-responsive.spec.ts --update-snapshots`

  Expected: les PNG sont créés sous `e2e/baselines/runner/` ; relancer sans `--update-snapshots` doit comparer les captures.

- [ ] **Step 4: Ajouter le smoke a11y/reduced-motion Playwright.**
...
- [ ] **Step 5: Exécuter le sous-ensemble Playwright.**
...
- [ ] **Step 6: Committer le livrable Playwright.**
...
- [ ] **Step 1: Rédiger CARACTERISATION.md.**

  Documenter chaque invariant avec test(s), source observée, résultat, données de fixture, et limites. Indiquer explicitement les deux `it.fails` pré-migration et le fait que les handlers réseau restent inchangés.

- [ ] **Step 2: Rédiger MATRICE-RESPONSIVE.md.**
...
- [ ] **Step 3: Vérifier la documentation contre le dépôt.**
...
- [ ] **Step 4: Committer le livrable documentaire.**
...
- [ ] **Step 1: Vérifier qu’aucun fichier chaud n’a changé.**
...
- [ ] **Step 2: Exécuter les gates dans l’ordre demandé.**

  Run: `npm run test`

  Expected: code 0 et zéro fichier/test en échec.
...
- [ ] **Step 3: Vérifier les commits et le statut final.**
