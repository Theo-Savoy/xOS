...
if (
  covered.size !== SECTEUR_VALUES.length ||
  SECTEUR_VALUES.some((s) => !covered.has(s))
) {
  throw new Error(
    'SECTEUR_FAMILIES must cover every SECTEUR_VALUES entry exactly once',
  );
}

export function secteurFamilyByValue(
  value: Secteur,
): SecteurFamily | undefined {
