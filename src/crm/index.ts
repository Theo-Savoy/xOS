...
export const TYPE_CLIENT_VALUES = [
  'Client inactif',
  'Client',
  'Prospect',
] as const;
...
export {
  SECTEUR_FAMILIES,
  secteurFamilyByValue,
  type SecteurFamily,
} from './secteurFamilies';
...
export const CONTACT_LIMIT_OPTIONS = [
  50,
  100,
  200,
  300,
...
export const RELANCE_DEFAULT_RESULTATS: ResultatCall[] = [
  'Appel non décroché',
  'Message répondeur',
];
...
export const PIPE_DECROCHE: ResultatCall[] = [
  'Appel décroché',
  'Appel argumenté',
  'RDV planifié',
];
...
export const PIPE_ARGUMENTE: ResultatCall[] = [
  'Appel argumenté',
  'RDV planifié',
];
...
function exclureSiPlusDe(
  value: unknown,
): { appels: number; sur_jours: number } | null {
...
export function normalizeFilterTree(raw: unknown): FilterTree {
  ...
  const entreprise = (
    source.entreprise &&
    typeof source.entreprise === 'object' &&
    !Array.isArray(source.entreprise)
      ? source.entreprise
      : {}
  ...
  const relance = (
    source.relance &&
    typeof source.relance === 'object' &&
    !Array.isArray(source.relance)
      ? source.relance
      : {}
  ) as Record<string, unknown>;
  ...
  return {
    entreprise: {
      ...
      opp_ouverte: triStateOr(
        defaults.entreprise.opp_ouverte,
        entreprise.opp_ouverte,
      ),
      opp_perdue: triStateOr(
        defaults.entreprise.opp_perdue,
        entreprise.opp_perdue,
      ),
    ...
    relance: {
      jamais_appele: triStateOr(
        defaults.relance.jamais_appele,
        relance.jamais_appele,
      ),
      dernier_appel_avant_jours: nullablePositiveInt(
        relance.dernier_appel_avant_jours,
      ),
      dernier_appel_dans_jours: nullablePositiveInt(
        relance.dernier_appel_dans_jours,
      ),
