...
export const MEDDIC_OPTIONS: Record<MeddicCategory, readonly MeddicOption[]> = {
  interet_produit: [
    ...
    {
      label: 'Intérêt produit mais pas prioritaire',
      value: 'Intérêt produit mais pas prioritaire',
    },
  ],
  maturite: [
  ...
  concurrence: [
    ...
    {
      label: 'Notre solution déjà en place',
      value: 'Notre solution déjà en place',
    },
...
export const RESULTAT_TO_MEDDIC_CATEGORIES: Record<
  ResultatCall,
  readonly MeddicCategory[]
> = {
...
function extractDetail(storedValue: string, option: MeddicOption): string {
  if (
    option.value === 'Budget validé pour tel mois' &&
    storedValue.startsWith('Budget validé pour ')
  ) {
    return storedValue.slice('Budget validé pour '.length);
  }
  if (
    option.value === 'Contrat expire telle date' &&
    storedValue.startsWith('Contrat expire ')
  ) {
...
function MeddicSection({
  ...
  const selectedOption =
    options.find((opt) => opt.value === pendingOption) ?? options[0];
  const needsDetail = selectedOption?.needsDetail ?? false;
  const canConfirm =
    pendingOption !== PLACEHOLDER_VALUE &&
    (!needsDetail || pendingDetail.trim().length > 0);
  ...
  return (
    ...
    >
      <div className="calls-medic-section__head">
        ...
        >
          ...
          {locked ? (
            ...
            <span className="calls-medic-section__placeholder">
              Non renseigné
            </span>
          )}
        </Button>
      ...
      {isOpen && (
        <div
          id={panelId}
          className="calls-medic-section__panel"
          role="region"
        ...
        >
          <Select
            ...
            renderValue={(selected) =>
              selected[0]?.value === PLACEHOLDER_VALUE
                ? 'Non renseigné'
                : selected[0]?.label
            }
          />
...
export function NoteTemplateSections({
  ...
  const [editingCategory, setEditingCategory] = useState<MeddicCategory | null>(
    null,
  );
  const [pendingOptions, setPendingOptions] = useState<
    Partial<Record<MeddicCategory, string>>
  >({});
  ...
  const getPendingDetail = useCallback(
    (category: MeddicCategory): string => {
      if (pendingDetails[category] !== undefined)
        return pendingDetails[category]!;
      const stored = parsed.categories[category];
      if (stored) {
        const match = findOptionForValue(category, stored);
  ...
  return (
    <div
      className="calls-medic-sections"
      role="group"
      aria-label="Modèles de note MEDDIC"
    >
        ...
        return (
          <MeddicSection
            ...
            onOptionChange={(nextValue) => {
              ...
              const option = MEDDIC_OPTIONS[category].find(
                (opt) => opt.value === nextValue,
              );
              if (
                option &&
                !option.needsDetail &&
                nextValue !== PLACEHOLDER_VALUE
              ) {
