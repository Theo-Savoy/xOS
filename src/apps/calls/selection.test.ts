...
import {
  buildPreviewContactList,
  canSelectContact,
  selectIdsWithCompanyCap,
  titlePriority,
} from './selection';
...
describe('titlePriority', () => {
  it('ranks dirigeants and directeurs above chargés', () => {
    expect(titlePriority('Directeur formation')).toBeGreaterThan(
      titlePriority('Responsable formation'),
    );
    expect(titlePriority('Responsable RH')).toBeGreaterThan(
      titlePriority('Chargé de formation'),
    );
    expect(titlePriority('DRH adjoint')).toBeGreaterThan(
      titlePriority('Chef de projet'),
    );
...
describe('buildPreviewContactList', () => {
  ...
  it('prefers directeurs within each company when capping', () => {
    ...
    expect(
      buildPreviewContactList(contacts, 10, 1)
...
describe('selectIdsWithCompanyCap', () => {
  it('selects everyone when there is no cap', () => {
    expect([...selectIdsWithCompanyCap(contacts, null)].sort()).toEqual([
      'c1',
      'c2',
      'c3',
      'c4',
  ...
  it('prefers directeurs / responsables when capping to 1 per company', () => {
    expect([...selectIdsWithCompanyCap(contacts, 1)].sort()).toEqual([
      'c2',
      'c4',
      'c5',
    ]);
  ...
  it('prefers the two highest titles when capping to 2', () => {
    expect([...selectIdsWithCompanyCap(contacts, 2)].sort()).toEqual([
      'c2',
      'c3',
      'c4',
      'c5',
  ...
  it('respects an eligibility set (dedup exclude)', () => {
    ...
    expect([...selectIdsWithCompanyCap(contacts, 1, eligible)].sort()).toEqual([
      'c3',
      'c4',
    ]);
  });
