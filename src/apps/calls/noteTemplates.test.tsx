...
describe('parseMeddicNote / upsertMeddicCategory', () => {
  ...
  it('replaces an existing category instead of duplicating', () => {
    ...
    const next = upsertMeddicCategory(
      initial,
      'douleur',
      'Pas de douleur exprimée',
    );
  ...
  it('formats budget month detail into a single structured value', () => {
    expect(formatMeddicValue('Budget validé pour tel mois', 'mars')).toBe(
      'Budget validé pour mars',
    );
  });
});
...
describe('RESULTAT_TO_MEDDIC_CATEGORIES', () => {
  it('exposes only timing for unreached outcomes', () => {
    expect(RESULTAT_TO_MEDDIC_CATEGORIES['Appel non décroché']).toEqual([
      'timing',
    ]);
    expect(RESULTAT_TO_MEDDIC_CATEGORIES['Message répondeur']).toEqual([
      'timing',
    ]);
  });
...
describe('NoteTemplateSections', () => {
  it('renders all applicable sections for the outcome', () => {
    ...
    expect(
      screen.getByRole('group', { name: 'Modèles de note MEDDIC' }),
    ).toBeTruthy();
  ...
  it('keeps other sections visible after selecting Douleur', async () => {
    ...
    await user.click(
      screen.getByRole('option', { name: 'Pas de douleur exprimée' }),
    );
  ...
  it('shows a locked section summary after validation', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Modifier Douleur' }),
    ).toBeTruthy();
  });
  ...
  it('keeps budget month detail in the structured note', async () => {
    ...
    await user.click(
      screen.getByRole('option', { name: 'Budget validé pour tel mois' }),
    );
  ...
  it('does not overwrite manual text when adding a structured section', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Choisir Douleur' }));
    await user.click(
      screen.getByRole('option', { name: 'Douleur conformité' }),
    );
  ...
  it('keeps every option reachable for a category', async () => {
    ...
    await user.click(
      within(panel).getByRole('button', { name: 'Choisir Timing' }),
    );
