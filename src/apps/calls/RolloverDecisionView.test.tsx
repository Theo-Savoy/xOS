...
describe('RolloverDecisionView', () => {
  it('uses action buttons per contact and requires confirmation before bulk removal', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Contacter Alice Martin' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Retirer Alice Martin' }),
    ).toBeTruthy();
    await user.click(
    ...
    expect(
      screen.getByRole('dialog', { name: 'Confirmer le retrait' }),
    ).toBeTruthy();
    await user.click(
      screen.getByRole('button', { name: 'Retirer les 2 contacts' }),
    );
