...
describe('EventPanel', () => {
  it('initializes datetime-local in local time rather than slicing a UTC string', () => {
    ...
    render(
      <EventPanel contactName="Alice" loading={false} onSubmit={vi.fn()} />,
    );
    ...
    expect(
      (screen.getByLabelText('Date & heure') as HTMLInputElement).value,
    ).toBe(localValue);
  });
  ...
  it('defaults to discovery subject and 60 min for prospection', async () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Rdv découverte prospect/i })
        .getAttribute('aria-pressed'),
    ).toBe('true');
    expect(
      screen.queryByRole('button', { name: /Rdv détection enjeux client/i }),
    ).toBeNull();
    expect(
      screen
        .getByRole('button', { name: /60\s*min/i })
        .getAttribute('aria-pressed'),
    ).toBe('true');
    await user.click(
      screen.getByRole('button', { name: /enregistrer le rdv/i }),
    );
    expect(onSubmit).toHaveBeenCalledWith(expect.any(String), 60, {
  ...
  it('defaults to detection enjeux for a Client account in prospection', () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Rdv détection enjeux client/i })
        .getAttribute('aria-pressed'),
    ).toBe('true');
    expect(
      screen.queryByRole('button', { name: /Rdv découverte prospect/i }),
    ).toBeNull();
  });
  ...
  it('defaults SDR attribution to a commercial colleague', async () => {
        ...
        team={[
          {
            user_id: 'user-1',
            label: 'Yanis',
            sf_user_id: '005Sb000007b6dWIAQ',
          },
          {
            user_id: 'user-2',
            label: 'Christophe',
            sf_user_id: '005000000000002',
          },
    ...
    expect(
      within(group)
        .getByRole('radio', { name: 'Christophe' })
...
describe('RunnerView', () => {
  ...
  it('keeps the logged contact in the prioritized event panel', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Finaliser le RDV — Alice Martin' }),
    ).toBeTruthy();
  });
  ...
  it('shows title, LinkedIn and result buttons on the contact card', async () => {
    ...
    expect(
      screen
        .getByRole('link', { name: 'Ouvrir sur LinkedIn' })
        .getAttribute('href'),
    ).toBe('https://linkedin.com/in/bob');
    expect(
      screen.getByRole('button', { name: 'Appel argumenté' }),
    ).toBeTruthy();
  ...
  it('auto-opens the first pending fiche with next-contact and recall hints', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 3, name: 'Bob Durand' }),
    ).toBeTruthy();
  ...
  it('logs with ⌘↵ for a planned meeting via the Event panel', async () => {
    ...
    expect(onLogRdvAndNext.mock.calls[0][1]).toMatchObject({
  ...
  it('shows Event panel inline when RDV planifié is selected', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Détails du RDV' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Consigner appel + RDV & suivant' }),
    ).toBeTruthy();
    expect(
      screen.queryByRole('button', { name: 'Consigner & suivant' }),
    ).toBeNull();
  });
  ...
  it('exposes poste, email and phone in the session list', async () => {
    ...
    expect(
      screen.getAllByRole('link', { name: '0102030405' }).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByRole('link', { name: 'alice@acme.fr' }).length,
    ).toBeGreaterThan(0);
    expect(screen.getAllByText('Responsable formation').length).toBeGreaterThan(
      0,
    );
  ...
  it('shows email on the contact fiche', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Fiche' }));
    expect(
      screen.getByRole('link', { name: 'bob@acme.fr' }).getAttribute('href'),
    ).toBe('mailto:bob@acme.fr');
  });
  ...
  it('does not show previous contact CRM history when context is stale', () => {
    ...
    const historyPanel = screen
      .getByRole('heading', { name: "Historique d'appels" })
      .closest('.calls-context-panel');
    expect(historyPanel).toBeTruthy();
    expect(
      within(historyPanel as HTMLElement).queryByText('Appel décroché'),
    ).toBeNull();
  });
  ...
  it('contains each expanded CRM context list independently', async () => {
    ...
    const historyPanel = screen
      .getByRole('heading', { name: "Historique d'appels" })
      .closest('.calls-context-panel');
    const opportunitiesPanel = screen
      .getByRole('heading', { name: 'Opportunités du compte' })
      .closest('.calls-context-panel');
    expect(historyPanel).toBeTruthy();
    expect(opportunitiesPanel).toBeTruthy();
    expect(
      within(historyPanel as HTMLElement).getAllByRole('listitem'),
    ).toHaveLength(5);
    expect(
      within(opportunitiesPanel as HTMLElement).getAllByRole('listitem'),
    ).toHaveLength(5);

    await user.click(
      within(historyPanel as HTMLElement).getByRole('button', {
        name: 'Voir tout (6)',
      }),
    );

    const expandedHistoryList = within(historyPanel as HTMLElement).getByRole(
      'list',
    );
    expect(
      expandedHistoryList.classList.contains('calls-context-list--expanded'),
    ).toBe(true);
    expect(
      within(historyPanel as HTMLElement).getAllByRole('listitem'),
    ).toHaveLength(6);
    ...
    expect(
      within(opportunitiesPanel as HTMLElement).getByRole('button', {
        name: 'Voir tout (6)',
      }),
    ).toBeTruthy();
  });
  ...
  it('lets the user schedule a recall after an answered call', async () => {
    ...
    expect(
      screen.getByRole('group', { name: 'Choisir la date de rappel' }),
    ).toBeTruthy();
    expect(screen.getByRole('button', { name: /\+3 j/i })).toBeTruthy();
    await user.click(
      screen.getByRole('button', { name: /Consigner & suivant/i }),
    );
  ...
  it('allows skipping recall on unanswered without NPA', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Appel non décroché' }),
    );
    expect(
      (screen.getByLabelText(/Planifier un rappel/i) as HTMLInputElement)
        .checked,
    ).toBe(true);
    await user.click(screen.getByLabelText(/Planifier un rappel/i));
  ...
  it('reschedules a recall from the recall queue', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Fiche' }));
    await user.click(
      screen.getByRole('button', { name: 'Modifier la date de rappel' }),
    );
    const dialog = screen.getByRole('dialog', {
      name: 'Modifier la date de rappel',
    });
  ...
  it('bulk-reschedules multiple recalls from the list selection', async () => {
    ...
    const dialog = await screen.findByRole('dialog', {
      name: 'Reporter les rappels',
    });
    await user.click(
      within(dialog).getByRole('button', { name: "Aujourd'hui" }),
    );
  ...
  it('opens the command bar with ⌘K and runs a resultat action', async () => {
    ...
    expect(
      screen.getByRole('dialog', { name: 'Command bar Combo' }),
    ).toBeTruthy();
    await user.click(screen.getByRole('option', { name: /Appel décroché/i }));
    expect(
      screen
        .getByRole('button', { name: /Appel décroché/i })
        .getAttribute('aria-pressed'),
    ).toBe('true');
  });
  ...
  it('sets recall delay with Shift+digit via keyboard code (AZERTY-safe)', () => {
    ...
    expect(
      screen.getByRole('button', { name: '+3 j' }).getAttribute('aria-pressed'),
    ).toBe('true');
  });
  ...
  it('opens shortcut help with ? and closes with Escape', () => {
    ...
    expect(
      screen.getByRole('dialog', { name: 'Aide raccourcis Combo' }),
    ).toBeTruthy();
    fireEvent.keyDown(document, { key: 'Escape' });
    expect(
      screen.queryByRole('dialog', { name: 'Aide raccourcis Combo' }),
    ).toBeNull();
  });
  ...
  it('ignores digit shortcuts while typing in comments', async () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Appel non décroché/i })
        .getAttribute('aria-pressed'),
    ).toBe('true');
  ...
  it('keeps the in-progress result and comments when changing the default recall delay', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Appel décroché' }));
    await user.type(
      screen.getByPlaceholderText("Notes sur l'appel…"),
      'À rappeler après validation',
    );
    ...
    expect(
      screen
        .getByRole('button', { name: 'Appel décroché' })
        .getAttribute('aria-pressed'),
    ).toBe('true');
  ...
  it('opens continuation session panel from Reporter with date', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Liste' }));
    expect(
      screen.queryByRole('button', { name: /Créer séance #2/i }),
    ).toBeNull();
    await user.click(
      screen.getByRole('button', { name: /Sélectionner \(2\)/i }),
    );
    ...
    await user.click(
      screen.getByRole('button', { name: /Créer Prospection Lyon #2/i }),
    );
  ...
  it('opens defer panel for Reporter', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /Créer Séance test #2/i }),
    ).toBeTruthy();
  });
  ...
  it('advances focus after Consigner & suivant when parent clears focusedContactId', async () => {
    ...
    expect(
      screen.getByRole('heading', { level: 3, name: 'Claire' }),
    ).toBeTruthy();
...
describe('RecapView', () => {
  it('uses the GET outcome field and announces a follow-up error', () => {
    ...
    expect(screen.getByRole('alert').textContent).toContain(
      'Aucun contact ne nécessite de relance.',
    );
  });
  ...
  it('prefills a readable name suggestion and a date for the follow-up session, next to the non-contacted list', async () => {
    ...
    const skippedListIndex = screen
      .getByText('Non contactés — reportés en follow-up')
      .compareDocumentPosition(
        screen.getByRole('button', { name: 'Préparer la relance' }),
      );
    ...
    expect(onCreateFollowUp).toHaveBeenCalledWith(
      nameInput.value,
      expect.stringMatching(/^\d{4}-\d{2}-\d{2}$/),
    );
  });
...
describe('SessionsView hub filters', () => {
  ...
  it('confirms session deletion in a custom modal', async () => {
    ...
    expect(
      screen.queryByRole('dialog', { name: 'Supprimer la séance' }),
    ).toBeNull();
...
describe('PicklistMultiSelect', () => {
  it('associates its visible label with the searchable input', () => {
    ...
    expect(
      screen
        .getByRole('searchbox', { name: 'Secteurs' })
  ...
  it('shows legacy free-text values as removable obsolete chips', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Retirer Secteur inventé' }),
    );
...
describe('call targeting copy and controls', () => {
  ...
  it('uses CRM-generic copy and exposes dedup toggle state', () => {
        ...
        <FilterBuilder
          ...
          team={[
            ...
            },
            {
              user_id: 'user-6',
              label: 'Théo Savoy',
              sf_user_id: '005AZ000000X5nDYAS',
            },
          ]}
        />
        <DedupBanner
          dedup={[
            { sf_contact_id: '003000000000001', in_session_of: 'Séance A' },
          ]}
          mode="avertir"
          onModeChange={vi.fn()}
        />
    ...
    expect(
      screen.getByRole('button', { name: 'Christophe Hirtz' }),
    ).toBeTruthy();
  ...
  it('shows guidance when lost is set to Non', () => {
    ...
    expect(
      nonButtons.every((btn) => !(btn as HTMLButtonElement).disabled),
    ).toBe(true);
  });
...
describe('preview selection and enriched rows', () => {
  ...
  it('can split a normal contact list with the shared audience packing contract', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Taille cible par séance'), {
      target: { value: '1' },
    });
