...
export function RecallFields({
  ...
  const activePreset = RECALL_PRESETS.find(
    (preset) => addDaysIso(preset.days) === recallAt,
  )?.days;
  ...
  return (
    <div className="calls-recall" role="group" aria-label="Rappel">
      ...
      {scheduleRecall ? (
        <div
          className="calls-recall__track"
          role="group"
          aria-label="Choisir la date de rappel"
        >
          <div
            className="calls-recall__presets"
            role="group"
            aria-label="Délai rapide"
          >
          ...
          <span className="calls-recall__or" aria-hidden="true">
            ou
          </span>
          <DatePicker
            compact
        ...
        <p className="calls-muted calls-recall__skip-hint">
          Pas de rappel cette fois — le contact reste appelable (contrairement
          au NPA).
        </p>
      )}
    </div>
