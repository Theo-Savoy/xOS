...
function hubResponse() {
  return Promise.resolve(
    new Response(JSON.stringify(mockHub), { status: 200 }),
  );
}
...
describe('CallManagerApp component', () => {
      ...
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls' && init?.method === 'POST') {
          ...
          if (body.action === 'accounts_search') {
            return Promise.resolve(
              new Response(
                JSON.stringify({ accounts: [account], truncated: false }),
                { status: 200 },
              ),
            );
          }
  ...
  it('opens recap follow-up sessions in the pre-session flow', async () => {
    ...
    await user.click(
      await screen.findByRole('button', { name: /Préparer la relance/i }),
    );

    expect(
      await screen.findByRole('heading', { name: 'Prospection Lyon #2' }),
    ).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Choisir le cap' })).toBeTruthy();
  });
  ...
  it('keeps the runner objective read-only and sourced from the session', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url === '/api/calls?session_id=1') {
        return Promise.resolve(
          new Response(
            JSON.stringify({ session: activeSession, contacts: [contact] }),
            { status: 200 },
          ),
        );
      }
    ...
    expect(
      screen.getByRole('progressbar', { name: 'Progression RDV : 0 sur 4' }),
    ).toBeTruthy();
  ...
  it('retiring 5 contacts in a row triggers only one full session refetch (debounce)', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls?session_id=1') {
          ...
          return Promise.resolve(
            new Response(JSON.stringify({ session: activeSession, contacts }), {
              status: 200,
            }),
          );
        }
    ...
    for (const contact of contacts) {
      await user.click(
        screen.getByLabelText(`Sélectionner ${contact.contact_name}`),
      );
  ...
  it('removes a contact locally before the remove API resolves', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(contact.contact_name)).toBeTruthy(),
    );
  ...
  it('closes an overdue active session and opens a decision screen for pending contacts', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls?resource=hub') {
          return Promise.resolve(
            new Response(JSON.stringify({ ...mockHub, sessions: [overdue] }), {
              status: 200,
            }),
          );
        }
    ...
    expect(
      await screen.findByRole('heading', {
        name: 'Décider du devenir des contacts',
      }),
    ).toBeTruthy();
    ...
    await user.click(screen.getByRole('button', { name: 'Retirer' }));
    await user.click(
      screen.getByRole('button', { name: 'Appliquer les décisions' }),
    );
    await user.click(
      screen.getByRole('button', { name: 'Retirer le contact' }),
    );
    await waitFor(() =>
      expect(postedActions).toEqual(['complete_session', 'remove_contact']),
    );
  });
  ...
  it('recalculates the detailed preview automatically after a filter change, ignoring stale responses', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls' && init?.method === 'POST') {
          ...
          if (body.count_only) {
            return Promise.resolve(
              new Response(JSON.stringify({ count: 3, capped: false }), {
                status: 200,
              }),
            );
          }
    ...
    await waitFor(() =>
      expect(screen.getByText('Contact récent')).toBeTruthy(),
    );
    ...
    vi.mocked(global.fetch).mockImplementation(
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls' && init?.method === 'POST') {
          ...
          return Promise.resolve(
            new Response(JSON.stringify({ error: 'sf_query_error' }), {
              status: 502,
            }),
          );
        }
  ...
  it('warns about a failed NPA sync without blocking runner progression', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Consigner & suivant' }),
    );
  ...
  it('loads a new context when the focused contact changes', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        contextFetches.some((u) => u.includes('context_contact_id=101')),
      ).toBe(true);
    });
    ...
    await waitFor(() => {
      ...
      expect(
        contextFetches.some((u) => u.includes('context_contact_id=102')),
      ).toBe(true);
    });
  });
  ...
  it('logs selected contacts in waves of four and aggregates failures', async () => {
    ...
    vi.mocked(global.fetch).mockImplementation(
      (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url === '/api/calls?resource=recalls')
          return Promise.resolve(
            new Response(JSON.stringify({ recalls: [] }), { status: 200 }),
          );
        if (url === '/api/calls?session_id=1') {
          sessionFetches += 1;
          return Promise.resolve(
    ...
    resolvers[2](
      new Response(JSON.stringify({ error: 'contact_already_processed' }), {
        status: 409,
      }),
    );
    resolvers[3](
    ...
    expect(screen.getByRole('alert').textContent).toContain(
      '6 consignés, 1 en échec — liste actualisée',
    );
  });
});
