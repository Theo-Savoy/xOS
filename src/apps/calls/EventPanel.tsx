...
export const EventPanel = forwardRef<EventPanelHandle, EventPanelProps>(
  function EventPanel(
    {
      contactName,
      loading,
    ...
    const assignees = useMemo(() => {
      const others = team.filter(
        (member) => member.sf_user_id && member.sf_user_id !== currentSfUserId,
      );
      if (!sdr) return others;
      return others.filter((member) =>
        isRdvAssigneeCandidate(member.sf_user_id),
      );
    ...
    const handleSubmit = () => {
      ...
      if (
        !RDV_DURATION_PRESETS.includes(
          durationMin as (typeof RDV_DURATION_PRESETS)[number],
        )
      ) {
      ...
      if (sdr) {
        if (
          !ownerSfUserId ||
          !assignees.some((member) => member.sf_user_id === ownerSfUserId)
        ) {
    ...
    const wrapperClass = ['calls-event-panel', className]
      .filter(Boolean)
      .join(' ');
    ...
    return (
      <Wrapper className={wrapperClass}>
        ...
        <div className="calls-fb-control" role="group" aria-label="Type de RDV">
            ...
            {subjects.map((subject) => (
              ...
              >
                ...
                {subject.countsForLundi && (
                  <span
                    className="calls-rdv-subjects__badge"
                    title="Compte pour le KPI Lundi"
                  >
                    Lundi
                  </span>
                )}
          ...
          {selectedSubject.countsForLundi ? (
            <p className="calls-muted calls-rdv-subjects__hint">
              Compte dans le bilan Lundi.
            </p>
          ) : (
            <p className="calls-muted calls-rdv-subjects__hint">
              Hors KPI Lundi (suivi / soutenance).
            </p>
          )}
        </div>
        ...
        <div
          className="calls-fb-control"
          role="group"
          aria-label="Durée du RDV"
        >
        ...
        {error && (
          <p role="alert" aria-live="assertive" className="calls-error">
            {error}
          </p>
        )}
        <Button
