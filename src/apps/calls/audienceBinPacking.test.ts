...
import {
  packAccountsIntoSessions,
  type PackableAccount,
} from './audienceBinPacking';

function account(
  id: string,
  name: string,
  contactCount: number,
): PackableAccount<{ id: string }> {
  return {
    id,
    name,
...
describe('packAccountsIntoSessions', () => {
  it('never splits a single account across sessions, even past the target size', () => {
    const groups = packAccountsIntoSessions(
      [account('a1', 'Big Corp', 200)],
      50,
      5,
    );
  ...
  it('groups the biggest accounts first and stays under the 20% tolerance', () => {
    const accounts = [
      account('a1', 'ACME Corp', 40),
      account('a2', 'ACME Subsidiary', 25),
      account('a3', 'Globex', 38),
      account('a4', 'Wayne', 22),
    ];
  ...
  it('returns groups ordered from biggest to smallest', () => {
    ...
    for (let i = 1; i < groups.length; i++) {
      expect(groups[i - 1].totalContacts).toBeGreaterThanOrEqual(
        groups[i].totalContacts,
      );
    }
  });
});
