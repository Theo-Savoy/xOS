...
export const PALIER_ORDER: PalierId[] = [
  'bronze',
  'argent',
  'or',
  'platine',
...
export function progressToNext(
  ...
  const next =
    currentIndex + 1 < PALIER_ORDER.length
      ? PALIER_ORDER[currentIndex + 1]
      : null;
  ...
  const pctToNext =
    span > 0
      ? Math.max(0, Math.min(100, ((value - previousThreshold) / span) * 100))
      : 100;
  ...
  return {
    ...
    next,
    pctToNext,
    valueToNext: Math.max(0, nextThreshold - value),
  };
}

export function detectPaliers(
  previousXp: ComboXp,
  newXp: ComboXp,
): AxePalier[] {
...
function buildDedupeKey(
  ...
  if (axis === 'vitesse' && options.actionId)
    return `vitesse:${options.actionId}:${dateParis}`;
  return null;
}
...
export function palierForCount(
  axis: ComboXpAxis,
  count: number,
): string | null {
...
export type ComboStreakSummary = {
  id: ComboStreakId;
  label: string;
  days: number;
  palier: string | null;
...
export function summarizeComboXp(userId: string): ComboXpSummary {
  ...
  const lastBadgeId =
    xp.badges.length > 0 ? xp.badges[xp.badges.length - 1] : null;
  const lastBadge = lastBadgeId
    ? { id: lastBadgeId, label: badgeLabel(lastBadgeId) }
    : null;
