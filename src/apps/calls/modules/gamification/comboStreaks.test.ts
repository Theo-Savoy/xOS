...
describe('computeStreak', () => {
  ...
  it('breaks the streak when a day is missing', () => {
    const result = computeStreak(
      ['2026-07-14', '2026-07-15', '2026-07-17', '2026-07-18'],
      '2026-07-18',
    );
    expect(result.currentDays).toBe(2);
  });
  ...
  it('tracks bestEver independently of the current streak', () => {
    const result = computeStreak(
      ['2026-07-01', '2026-07-02', '2026-07-03', '2026-07-04', '2026-07-17'],
      '2026-07-18',
    );
  ...
  it('BUG-07: does not throw RangeError on corrupted/invalid date strings', () => {
    // Corrupted localStorage entries (partial write, manual edit) must not crash the streak calculation.
    expect(() => computeStreak(['invalid', '2026-07-17'], '2026-07-18')).not.toThrow();
    expect(() => computeStreak(['', '2026-07-17'], '2026-07-18')).not.toThrow();
    expect(() => computeStreak(['oopsT00:00:00Z'], '2026-07-18')).not.toThrow();
    // Invalid entries are ignored, valid ones still count.
    const result = computeStreak(['garbage', '2026-07-17', '2026-07-18'], '2026-07-18');
    expect(result.currentDays).toBe(2);
  });
});
