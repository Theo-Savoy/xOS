...
function isPlannedSession(session: SessionSummary, today: string): boolean {
  return (
    ...
    Boolean(session.scheduled_for && session.scheduled_for > today)
  );
}
...
function matchesSchedule(
  ...
  if (filter === 'upcoming')
    return session.status === 'active' && !isPlannedSession(session, today);
...
export function SessionsView({
  ...
  const [scheduleFilter, setScheduleFilter] =
    useState<ScheduleFilter>('upcoming');
  ...
  const kpis =
    (kpiPeriod === 'week' ? stats?.week : stats?.month) ?? emptyKpis();
  ...
  const hasActiveFilters =
    scheduleFilter !== 'upcoming' || typeFilter !== 'all';
    ...
    <div className="calls-view calls-hub">
      ...
      <section className="calls-hub-kpis" aria-label="Indicateurs">
          ...
          <div>
            ...
            {stats && (
              <p className="calls-hub-kpis__meta">
                Aujourd&apos;hui {stats.calls_today} · Semaine{' '}
                {stats.calls_week} · {stats.sessions_active} active
                {stats.sessions_active > 1 ? 's' : ''}
              </p>
            )}
          </div>
        ...
        <p className="calls-hub-kpis__secondary">
          RDV / argumenté{' '}
          <strong className="xos-numeric">
            {pct(kpis.rate_rdv_per_argumente)}
          </strong>
      ...
      <section className="calls-hub-sessions" aria-label="Séances">
            ...
            <div className="calls-seg" role="group" aria-label="Vue séances">
              ...
              >
                ...
                {recallsLoading ? (
                  ...
                  <span className="calls-seg__count xos-numeric">
                    {recallCount}
                  </span>
                ) : null}
              </Button>
        ...
        {loading && sessions.length === 0 && (
          <p className="calls-state">Chargement des séances…</p>
        )}
        {error && (
        ...
        {trulyEmpty && (
          <GlassCard className="calls-empty calls-empty--hero">
            ...
            <p>
              Composez une liste ciblée, planifiez une séance et suivez vos
              indicateurs ici.
            </p>
                  ...
                  <div className="calls-session-card__body">
                    ...
                    >
                      ...
                      <div className="calls-session-card__meta">
                        ...
                        {session.skipped > 0 && (
                          <span>
                            {session.skipped} non contacté
                            {session.skipped > 1 ? 's' : ''}
                          </span>
                        )}
                        {session.pending > 0 && session.status === 'active' && (
                          <span className="xos-numeric">
                            {session.pending} restants
                          </span>
                        )}
                      </div>
                    </Button>
        ...
        {!error && viewMode === 'calendar' && !trulyEmpty && (
          <GlassCard className="calls-calendar">
            ...
            {filteredEmpty && (
              <p className="calls-calendar__empty-hint">
                Aucune séance pour ces filtres sur le calendrier.
              </p>
            )}
