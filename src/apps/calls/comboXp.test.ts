...
describe('comboXp', () => {
  ...
  it('applyEvent reports paliersFranchis when a threshold is crossed', () => {
    ...
    const result = applyEvent(USER, 'shortcut');
    expect(result.paliersFranchis).toEqual([
      { axis: 'vitesse', palier: 'bronze' },
    ]);
  });
  ...
  describe('BUG-02: anti-abus dedup', () => {
    it('2 clics du même raccourci le même jour = 1 XP Vitesse', () => {
      const first = applyEvent(USER, 'shortcut', 1, {
        actionId: 'L',
        dateParis: '2026-07-19',
      });
      const second = applyEvent(USER, 'shortcut', 1, {
        actionId: 'L',
        dateParis: '2026-07-19',
      });
    ...
    it('2 crédits Régularité le même jour = 1 seul crédit (un streak par jour)', () => {
      const first = applyEvent(USER, 'day-logged', 1, {
        dateParis: '2026-07-19',
      });
      const second = applyEvent(USER, 'day-logged', 1, {
        dateParis: '2026-07-19',
      });
  ...
  describe('summarize* (lecture/présentation, ex-useComboXp)', () => {
    ...
    it('computes the current palier per axis from cumulative counts', () => {
      ...
      const summary = summarizeComboXp('user-1');
      expect(summary.axes.find((a) => a.id === 'vitesse')).toMatchObject({
        count: 30,
        palier: 'Argent',
      });
      expect(summary.axes.find((a) => a.id === 'impact')).toMatchObject({
        count: 70,
        palier: 'Argent',
      });
      expect(summary.axes.find((a) => a.id === 'regularite')).toMatchObject({
        count: 14,
        palier: 'Or',
      });
    });
    ...
    it('surfaces the most recently unlocked badge', () => {
      saveXp('user-1', {
        ...
        impact: 0,
        regularite: 1,
        badges: ['premier_pas', 'eclair'],
        lastSeen: '',
      });
      expect(summarizeComboXp('user-1').lastBadge).toEqual({
        id: 'eclair',
        label: '⚡ Éclair',
      });
    });
    ...
    it('reads streak counters independently per type', () => {
      ...
      expect(streaks.find((s) => s.id === 'classique')).toMatchObject({
        days: 14,
        palier: 'Or',
      });
      expect(streaks.find((s) => s.id === 'intense')).toMatchObject({
        days: 0,
        palier: null,
      });
    });
    ...
    it('keeps users isolated by storage key', () => {
      ...
      expect(
        summarizeComboXp('user-b').axes.find((a) => a.id === 'vitesse')?.count,
      ).toBe(0);
    });
  });
});
