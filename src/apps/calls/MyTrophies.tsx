...
import { badgeLabel, currentPalier, summarizeComboBadges, summarizeComboStreaks, useComboXp } from "./comboXp";
...
export function MyTrophies({ open, onClose, userId }: MyTrophiesProps) {
  ...
  const remainingBadges = ALL_BADGE_IDS.filter((id) => !unlockedBadgeIds.has(id));
            ...
            {xp.axes.map((axis) => {
              ...
              return (
                <li key={axis.id}>
                  ...
                  {axis.palier && (
                    <span className={`calls-palier calls-palier--${palierId}`}> · {axis.palier}</span>
                  )}
                </li>
              );
            })}
