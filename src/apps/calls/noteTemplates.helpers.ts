...
function matchStructuredLine(
  line: string,
): { category: MeddicCategory; value: string } | null {
...
export function rebuildMeddicNote(
  ...
  const structuredLines = orderedCategories
    ...
    .map(
      (category) =>
        `${CATEGORY_LABELS[category]} : ${categories[category]!.trim()}`,
    );
...
export function removeMeddicCategory(
  note: string,
  category: MeddicCategory,
): string {
  const { manual, categories } = parseMeddicNote(note);
...
export function formatMeddicValue(
  optionValue: string,
  detail?: string,
): string {
...
export function visibleMeddicCategories(
  ...
  const set = new Set<MeddicCategory>([
    ...resultatCategories,
    ...filledCategories,
  ]);
