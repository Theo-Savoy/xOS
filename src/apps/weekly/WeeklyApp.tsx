...
function QuarterGauge({ data }: { data: Quarter | undefined }) {
  ...
  return <div className="weekly-quarter">
    <div className="weekly-quarter-heading"><span>{data?.quarter || "Trimestre"}</span><small>Objectif</small></div>
...
function PersonCard({ owner, pulseSeries, pipelineSeries, quarter, delay, weekMode }: { owner: Owner; pulseSeries: Pulse[]; pipelineSeries: Pipeline[]; quarter: Quarter | undefined; delay: number; weekMode: boolean }) {
  const tracking = trackingOf(owner);
  const current = pulseSeries.at(-1)!;
  const previous = pulseSeries.at(-2);
  const currentPipeline = pipelineSeries.at(-1)!;
  const previousPipeline = pipelineSeries.at(-2);
  const badge = trackingBadge(tracking, owner.role);
  ...
  return <GlassCard className="weekly-pulse-card weekly-pulse-card--current" style={{ "--weekly-delay": `${delay}ms` } as React.CSSProperties}>
    ...
    {!weekMode && tracking !== "sdr" && <QuarterGauge data={quarter} />}
  </GlassCard>;
}
...
function TeamRollup({
  owners, pulseFor, pipelineFor, quarterFor, weekMode,
}: {
  owners: Owner[];
  ...
  const weekCount = pulseFor(owners[0]).length;
  const current = weekCount - 1;
  const previous = weekCount > 1 ? weekCount - 2 : -1;
  const metric = <T,>(pool: Owner[], seriesFor: (owner: Owner) => T[], pick: (row: T) => number) => {
    if (weekMode) {
      const value = sumAt(pool, seriesFor, current, pick);
      const prev = previous >= 0 ? sumAt(pool, seriesFor, previous, pick) : undefined;
      return { value, previous: prev };
    }
    const values = Array.from({ length: weekCount }, (_, index) => sumAt(pool, seriesFor, index, pick));
  ...
  const wonTypes = weekMode
    ? teamWonByType(sellers, pipelineFor, current)
    : Array.from({ length: weekCount }, (_, index) => teamWonByType(sellers, pipelineFor, index))
  ...
  const ownerTotal = (pick: (owner: Owner) => number) => owners
    ...
    .slice(0, 4);
  const meetingLeaders = ownerTotal((owner) => weekMode ? (pulseFor(owner).at(-1)?.meetings || 0) : pulseFor(owner).reduce((sum, row) => sum + row.meetings, 0));
  const wonLeaders = ownerTotal((owner) => weekMode ? (pipelineFor(owner).at(-1)?.won_amount || 0) : pipelineFor(owner).reduce((sum, row) => sum + row.won_amount, 0));
  ...
  return <section className="weekly-section">
    ...
    <GlassCard className="weekly-team-rollup">
      ...
      {!weekMode && <QuarterGauge data={quarter} />}
...
function TeamMetricTable({
  owners, weeks, pulseFor, pipelineFor, quarterFor,
}: {
  owners: Owner[];
  weeks: Week[];
  pulseFor: (owner: Owner) => Pulse[];
...
function ForecastChart({ weeks, history, ownerIds }: { weeks: Week[]; history: ForecastPoint[]; ownerIds: Set<string> }) {
  const data = weeks.map((week) => {
    ...
    return {
      ...
      signed: points.reduce((sum, point) => sum + (point.signed_to_date || 0), 0),
    };
  });
  ...
  return <GlassCard className="weekly-chart-card weekly-forecast-card">
    ...
    <p className="weekly-closing">Forecast (snapshot lundi) vs signé cumulé du trimestre{!hasForecast ? " · historique en cours" : ""}</p>
  </GlassCard>;
}
...
export default function WeeklyApp() {
  ...
  const switchPeriod = (next: PeriodMode) => {
    setPeriod(next);
    setDisplayMode(next === "quarter" ? "table" : "cards");
  ...
  return <main className={`weekly-app ${loading ? "weekly-app--loading" : ""}`}>
    ...
    {!hasActivity ? <GlassCard className="weekly-empty"><h3>Une semaine encore calme</h3><p>Les activités Salesforce apparaîtront ici au fil des saisies.</p><span>Consultez Call Manager pour enregistrer vos appels.</span></GlassCard> : <>
      {showTeamRollup && displayMode === "cards" && <TeamRollup owners={visibleOwners} pulseFor={pulseFor} pipelineFor={pipelineFor} quarterFor={quarterFor} weekMode={weekMode} />}
      {displayMode === "table" ? <>
        <section className="weekly-section"><div className="weekly-section-heading"><p>Rituel</p><h3>{weekMode ? "Cette semaine vs S−1" : "Trimestre en cours"}</h3></div><div className="weekly-tables weekly-view-transition">
          {showTeamRollup && <TeamMetricTable owners={visibleOwners} weeks={model.weeks} pulseFor={pulseFor} pipelineFor={pipelineFor} quarterFor={quarterFor} />}
          {visibleOwners.map((owner) => <MetricTable key={owner.sf_user_id} owner={owner} weeks={model.weeks} pulse={pulseFor(owner)} pipeline={pipelineFor(owner)} quarter={quarterFor(owner)} />)}
        </div></section>
        ...
        <section className="weekly-section"><div className="weekly-section-heading"><p>Pulse</p><h3>{weekMode ? "Cette semaine vs S−1" : "Qui a bougé ?"}</h3></div><div className="weekly-pulse-grid weekly-view-transition">{visibleOwners.map((owner, ownerIndex) => (
          <PersonCard key={owner.sf_user_id} owner={owner} pulseSeries={pulseFor(owner)} pipelineSeries={pipelineFor(owner)} quarter={quarterFor(owner)} delay={ownerIndex * 70} weekMode={weekMode} />
        ))}</div></section>
        ...
        {showCustomPipe && <CustomPipeSection pipe={customPipe} owners={visibleOwners} sellerIds={sellerIds} />}
        {showForecast && <section className="weekly-section"><div className="weekly-section-heading"><p>Effort</p><h3>Forecast vs réalisé</h3></div><ForecastChart weeks={model.weeks} history={forecastHistory} ownerIds={sellerIds} /></section>}
      </>}
    </>}
  </main>;
}
