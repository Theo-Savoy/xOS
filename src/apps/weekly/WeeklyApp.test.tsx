...
vi.mock('recharts', () => ({
  ...
  Scatter: ({ children }: { children?: React.ReactNode }) => (
    <div data-testid="opp-scatter">{children}</div>
  ),
...
const selfPayload = {
  ...
  pipeline: [
    ...
    },
    {
      ...
      week_start: '2026-07-06',
      generated_count: 2,
      generated_amount: 12000,
      won_count: 1,
      won_amount: 6000,
      won_by_type: { catalogue: 3000, sur_mesure: 2000, conseil: 1000 },
      won_arr_amount: 3000,
  ...
  follow_up_opps: [
    {
      ...
      name: 'Deal à pousser',
      sf_user_id: 'self',
      stage: 'Négo financière engagée',
      amount: 20000,
      probability: 50,
      expected: 10000,
      close_date: '2026-08-15',
      url: 'https://example.salesforce.com/lightning/r/Opportunity/006F/view',
    },
  ],
  stagnant_opps: [
    {
      id: '006S',
      name: 'Deal silencieux',
      sf_user_id: 'self',
      ...
      days_in_stage: 52,
      days_since_activity: 28,
      reasons: ['stage', 'silence'],
      url: 'https://example.salesforce.com/lightning/r/Opportunity/006S/view',
    },
  ],
  pace: {
    week_of_quarter: 1,
  ...
  custom_pipe: {
    ...
    months: [
      ...
      },
      {
        ...
        expected: 9000,
        count: 1,
        by_owner: { self: { amount: 18000, expected: 9000, count: 1 } },
      },
      {
        month: '2026-11',
        label: 'nov.',
...
const teamPayload = {
  ...
  owners: [
    ...
    },
    {
      sf_user_id: 'sdr',
      name: 'Yanis Agharbi',
      email: 'yanis@xos-learning.fr',
      role: 'commercial' as const,
      tracking: 'sdr' as const,
    },
    {
      sf_user_id: 'dg',
      name: 'Jérôme Bosio',
      email: 'jerome@xos-learning.fr',
      role: 'manager' as const,
      tracking: 'dg' as const,
    },
  ],
  pulse: [
    ...selfPayload.pulse,
  ...
  pipeline: [
    ...selfPayload.pipeline,
    {
      ...
      week_start: '2026-07-06',
      generated_count: 3,
      generated_amount: 18000,
      won_count: 2,
      won_amount: 9000,
      won_by_type: { catalogue: 4000, sur_mesure: 3000, conseil: 1000 },
      won_arr_amount: 4000,
      closing_rate_count: 0.67,
      closing_rate_amount: 0.5,
    },
    {
      sf_user_id: 'sdr',
    ...
    },
    {
      ...
      won_count: 1,
      won_amount: 15000,
      won_by_type: { catalogue: 0, sur_mesure: 15000, conseil: 0 },
      won_arr_amount: 0,
      closing_rate_count: null,
      closing_rate_amount: null,
    },
...
const quarterPayload = {
  ...
  range: { from: '2026-06-29', to: '2026-07-12' },
  prior_pulse: selfPayload.pulse.map((row) => ({
    ...row,
    meetings: Math.max(0, row.meetings - 1),
    calls: Math.max(0, row.calls - 1),
  })),
  prior_pipeline: selfPayload.pipeline.map((row) => ({
    ...row,
    generated_count: Math.max(0, row.generated_count - 1),
    won_amount: Math.max(0, row.won_amount - 1000),
  })),
};
...
beforeEach(() => {
  getSession.mockResolvedValue({
    data: {
      session: {
        access_token: 'token',
        user: { email: 'ada@xos-learning.fr' },
      },
    },
  });
  vi.stubGlobal(
    ...
    vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify(selfPayload), { status: 200 }),
      ),
  );
});
...
describe('Weekly Perf', () => {
  it('renders the Monday decision board with weighted and stagnant opps', async () => {
    ...
    expect(
      screen.getByRole('link', { name: 'Deal à pousser' }).getAttribute('href'),
    ).toBe('https://example.salesforce.com/lightning/r/Opportunity/006F/view');
  ...
  it('filters the team view down to one commercial', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(teamPayload), { status: 200 }),
        ),
    );
    render(<WeeklyApp />);
    ...
    expect(
      screen.queryByRole('heading', { level: 4, name: 'Yanis Agharbi' }),
    ).toBeNull();
  ...
  it("renders a commercial's week metrics with the team toggle", async () => {
    ...
    expect(
      screen.getByRole('button', { name: /snapshot de la période/i }),
    ).toBeTruthy();
  ...
  it('switches to quarter consolidated view', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(
        new Response(JSON.stringify(quarterPayload), { status: 200 }),
      );
    ...
    expect(
      fetchMock.mock.calls.some((call) =>
        String(call[0]).includes('period=quarter'),
      ),
    ).toBe(true);
  ...
  it('splits the volume chart into one mini chart per unit instead of a shared axis', async () => {
    ...
    expect(
      within(section).getByTestId('weekly-activity-mini-rdv'),
    ).toBeTruthy();
    expect(
      within(section).getByTestId('weekly-activity-mini-detections'),
    ).toBeTruthy();
    expect(
      within(section).getByTestId('weekly-activity-mini-calls'),
    ).toBeTruthy();
  ...
  it('drops the Appels mini chart when no seller logged calls', async () => {
    const quarterNoCallsPayload = {
      ...
      prior_pulse: quarterPayload.prior_pulse.map((row) => ({
        ...row,
        calls: 0,
      })),
    };
    const fetchMock = vi
      ...
      .mockResolvedValue(
        new Response(JSON.stringify(quarterNoCallsPayload), { status: 200 }),
      );
    vi.stubGlobal('fetch', fetchMock);
    render(<WeeklyApp />);
    await screen.findByText('Ada Lovelace');
    ...
    expect(
      within(section).queryByTestId('weekly-activity-mini-calls'),
    ).toBeNull();
  });
  ...
  it('shows consolidated team stats in team view', async () => {
    ...
    const rollup = screen
      .getByText(/Consolidé · S27/)
      .closest('.weekly-section') as HTMLElement;
    ...
    expect(
      await screen.findByRole('table', { name: /consolidé de l.équipe/i }),
    ).toBeTruthy();
  });
  ...
  it('keeps the display mode when switching to quarter', async () => {
    ...
    render(<WeeklyApp />);
    await screen.findByText('Ada Lovelace');
    expect(screen.getByRole('button', { name: 'Cards' }).className).toContain(
      'xos-btn--primary',
    );
    fireEvent.click(screen.getByRole('button', { name: 'Trimestre' }));
    expect(await screen.findByText(/FY27-Q1 · S1\/13/)).toBeTruthy();
    ...
    expect(
      screen.getByRole('heading', { name: 'Où en est le trimestre' }),
    ).toBeTruthy();
  });
  ...
  it('shows the full team roster without a DG badge', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Équipe' }));
    expect(
      screen.getByRole('heading', { level: 4, name: 'Grace Hopper' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('heading', { level: 4, name: 'Jérôme Bosio' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('heading', { level: 4, name: 'Yanis Agharbi' }),
    ).toBeTruthy();
    expect(screen.getByText('SDR')).toBeTruthy();
    expect(screen.getByText('Manager')).toBeTruthy();
    expect(screen.queryByText('DG')).toBeNull();
    expect(
      screen.queryByRole('checkbox', { name: 'Commerciaux seulement' }),
    ).toBeNull();
  });
  ...
  it('shows three visible CA breakdown bars sized from won_by_type', async () => {
    render(<WeeklyApp />);
    const card = (
      await screen.findByRole('heading', { level: 4, name: 'Ada Lovelace' })
    ).closest('.weekly-pulse-card') as HTMLElement;
    const breakdown = within(card).getByLabelText('Répartition du CA signé');
    const widthOf = (type: string) =>
      (
        breakdown
          .querySelector(`.weekly-breakdown-${type}`)
          ?.closest('.weekly-tip') as HTMLElement
      ).style.width;
  ...
  it('does not render the CA breakdown when wonAmount is 0', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(zeroWonPayload), { status: 200 }),
        ),
    );
    render(<WeeklyApp />);
    const card = (
  ...
  it('adds an Autres category and warns when won_by_type sum is below wonAmount', async () => {
      ...
      pipeline: selfPayload.pipeline.map((row) =>
        row.week === '2026-W28'
          ? {
              ...row,
              won_amount: 6000,
              won_by_type: { catalogue: 2000, sur_mesure: 1000, conseil: 500 },
            }
          : row,
      ),
    };
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(incoherentPayload), { status: 200 }),
        ),
    );
    render(<WeeklyApp />);
    const card = (
  ...
  it('shows SDR metrics without sales breakdown', async () => {
    ...
    render(<WeeklyApp />);
    fireEvent.click(await screen.findByRole('button', { name: 'Équipe' }));
    const sdrCard = screen
      .getByRole('heading', { level: 4, name: 'Yanis Agharbi' })
      .closest('.weekly-pulse-card');
    expect(sdrCard).toBeTruthy();
    expect(within(sdrCard as HTMLElement).getByText('RDV pris')).toBeTruthy();
    expect(
      within(sdrCard as HTMLElement).queryByLabelText(
        'Répartition du CA signé',
      ),
    ).toBeNull();
  });
  ...
  it('shows the Salesforce mapping warning as a banner', async () => {
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify({ ...selfPayload, warning: 'sf_user_unmapped' }),
            { status: 200 },
          ),
        ),
    );
  ...
  it('renders amont before pace and line lower on the page', async () => {
    ...
    expect(
      leading.compareDocumentPosition(rythme) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      rythme.compareDocumentPosition(line) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it('computes table totals and week-over-week deltas client-side', async () => {
    ...
    const table = screen.getByRole('table', {
      name: 'Suivi hebdomadaire de Ada Lovelace',
    });
    expect(
      within(table).getByRole('columnheader', { name: 'Total' }),
    ).toBeTruthy();
    expect(
      within(table).getByRole('columnheader', { name: /Écart vs S27/ }),
    ).toBeTruthy();
    expect(
      within(table).getByRole('row', { name: /RDV effectués/ }),
    ).toBeTruthy();
    expect(
      within(table).queryByRole('row', { name: /Pipe sur-mesure/ }),
    ).toBeNull();
    expect(within(table).getAllByRole('row')).toHaveLength(8);
    expect(within(table).queryByRole('row', { name: /Target/ })).toBeNull();
    expect(
      within(table.closest('.weekly-table-card') as HTMLElement).getByText(
        'Objectif',
      ),
    ).toBeTruthy();
  });
  ...
  it('keeps a zero-to-zero delta equal and labels a non-zero value over zero as new', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(zeroBaselinePayload), { status: 200 }),
        ),
    );
    render(<WeeklyApp />);
    ...
    const meetingsDelta = within(table)
      .getByRole('row', { name: /RDV effectués/ })
      .querySelector('td:last-child');
    const generatedDelta = within(table)
      .getByRole('row', { name: /Opps détectées/ })
      .querySelector('td:last-child');
  ...
  it('keeps the SDR ledger visible with zeros instead of a hard empty state', async () => {
    const emptySdrPayload = {
      ...
      quarter: teamPayload.quarter.map((row) => ({
        ...row,
        signed_to_date: 0,
        forecast: 0,
        custom_pipe: 0,
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify(emptySdrPayload), { status: 200 }),
        ),
    );
    render(<WeeklyApp />);
    fireEvent.click(await screen.findByRole('button', { name: 'Équipe' }));
    ...
    const table = await screen.findByRole('table', {
      name: 'Suivi hebdomadaire de Yanis Agharbi',
    });
  ...
  it('keeps the week selector after choosing a past week', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValue(
        new Response(JSON.stringify(pastPayload), { status: 200 }),
      );
    vi.stubGlobal('fetch', fetchMock);
    render(<WeeklyApp />);
    await screen.findByText('Ada Lovelace');
    ...
    expect(
      fetchMock.mock.calls.some((call) =>
        String(call[0]).includes('week_start=2026-06-29'),
      ),
    ).toBe(true);
  });
