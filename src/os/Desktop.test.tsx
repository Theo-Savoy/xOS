      ...
      table === 'desktop_shortcuts'
        ? {
            select: vi.fn().mockReturnValue({
              order: vi
                .fn()
                .mockResolvedValue({ data: shortcutRows, error: null }),
            }),
            ...
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                maybeSingle: vi.fn().mockResolvedValue({
                  data: {
                    role: 'admin',
                    sf_auth_connected_at: '2026-07-01T00:00:00Z',
                  },
                  error: null,
                }),
              }),
...
describe('Desktop', () => {
  ...
  it('opens two dock applications in simultaneous windows', async () => {
    ...
    render(
      <Desktop userEmail="theo@xos-learning.fr" accessToken="test-token" />,
    );
  ...
  it('minimizes a window and restores it from the dock', async () => {
    ...
    await user.click(
      await screen.findByRole('button', { name: 'Réduire Combo' }),
    );
    expect(
      screen.getByRole('dialog', { name: 'Combo' }).closest('.xos-rnd-window')
        ?.className,
    ).toContain('xos-rnd-window--minimized');

    await user.click(screen.getByRole('button', { name: 'Restaurer Combo' }));
    expect(
      screen.getByRole('dialog', { name: 'Combo' }).closest('.xos-rnd-window')
        ?.className,
    ).not.toContain('xos-rnd-window--minimized');
  });
  ...
  it('toggles maximize and closes a window with its traffic-light controls', async () => {
    ...
    await user.click(
      await screen.findByRole('button', { name: 'Agrandir Labo' }),
    );
  ...
  it('removes a desktop shortcut', async () => {
    ...
    await user.click(
      await screen.findByRole('button', {
        name: 'Supprimer le raccourci Mes appels',
      }),
    );
  ...
  it('starts the authenticated Salesforce account-link flow', async () => {
    ...
    expect(navigate).toHaveBeenCalledWith(
      'https://login.salesforce.test/authorize',
    );
  });
  ...
  it('shows a reconnect CTA when Salesforce API is unreachable', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Reconnecter Salesforce' }),
    ).toBeTruthy();
  });
});
