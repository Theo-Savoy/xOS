  ...
  results: [
    {
      ...
      name: 'Acme Corp',
      detail: 'Tech · Alice',
      recordUrl: 'https://sf.lightning/r/Account/001AAA/view',
    },
    {
      type: 'Account',
      id: '001BBB',
      name: 'Beta Inc',
      detail: '',
      recordUrl: 'https://sf.lightning/r/Account/001BBB/view',
    },
    {
      type: 'Contact',
      id: '003CCC',
      ...
      recordUrl: 'https://sf.lightning/r/Contact/003CCC/view',
    },
    {
      type: 'Opportunity',
      id: '006DDD',
      name: 'Deal Alpha',
      detail: 'Qualification · Acme Corp',
      recordUrl: 'https://sf.lightning/r/Opportunity/006DDD/view',
    },
  ],
};
...
describe('Launcher', () => {
  ...
  it('sends Authorization: Bearer <accessToken> from props', async () => {
    ...
    await waitFor(() => {
      const searchCall = fetchSpy.mock.calls.find(
        (c: [string | URL | Request, RequestInit?]) =>
          typeof c[0] === 'string' && c[0].includes('/api/launcher'),
      );
  ...
  it('opens recordUrl in new tab with noopener,noreferrer', async () => {
    ...
    const item =
      screen.getByText('Acme Corp').closest('[cmdk-item]') ||
  ...
  it('calls onOpenApp when a local app is selected', async () => {
    ...
    const item =
      screen.getByText('Labo').closest('[cmdk-item]') ||
  ...
  it('debounces search requests (not one per keystroke)', async () => {
    ...
    await user.type(screen.getByRole('combobox'), 'ABCD', {
      delay: 1,
    } as Parameters<typeof user.type>[2]);
  ...
  it('resets loading spinner when /log association search drops below 2 chars', async () => {
    ...
    await user.keyboard('{Meta>}k{/Meta}');
    const logCmd =
      screen.getByText('/log').closest('[cmdk-item]') ||
    ...
    const recordSearch = screen.getByPlaceholderText(
  ...
  it('resets loading spinner when /create account search drops below 2 chars', async () => {
    ...
    await user.keyboard('{Meta>}k{/Meta}');
    const createCmd =
      screen.getByText('/create').closest('[cmdk-item]') ||
    ...
    const accountSearch = screen.getByPlaceholderText(
  ...
  describe('Commands and Inline Forms', () => {
    ...
    it('opens Labo when /clean command is selected', async () => {
      ...
      const cleanItem =
        screen.getByText(/Ouvrir Labo/).closest('[cmdk-item]') ||
    ...
    it('transitions to /log form and submits call notes successfully', async () => {
      ...
      localFetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({ error: null, success: true, taskId: '00T123' }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
      ...
      const recordSearchInput = screen.getByPlaceholderText(
        'Rechercher un enregistrement...',
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Note d'appel enregistrée avec succès !"),
        ).toBeTruthy();
      });

      // Verify fetch payload
      const logCall = localFetchSpy.mock.calls.find(
        (c) => c[0] === '/api/launcher',
      );
    ...
    it('transitions to /create form and submits contact successfully', async () => {
      ...
      localFetchSpy.mockResolvedValueOnce(
        new Response(
          JSON.stringify({ error: null, success: true, contactId: '003XYZ' }),
          {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          },
